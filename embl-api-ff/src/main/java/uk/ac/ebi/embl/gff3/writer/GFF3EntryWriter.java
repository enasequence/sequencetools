...
public class GFF3EntryWriter {
	...
	private boolean filterSegment(Long beginPosition, Long endPosition) {
		return (beginPosition == null || endPosition == null ||
	...
	private void writeFeature(Writer writer, Feature feature, String geneName, String locusTag,
			int parentID, Long beginPosition, Long endPosition) throws IOException {
		if (filterSegment(beginPosition, endPosition)) {
			return;
		}
		// Filter out gene and source features.
		if (feature.getName().equals("gene") || (!show.contains(SHOW_SOURCE) &&	
		...
		GFF3Writer.writeColumns(writer, 
  	...
  	writer.write(";");
  	String trackName = "";
  	if (show.contains(GROUP_GENE) && (geneName != null || locusTag != null) &&
  		!feature.getName().equals("source")) {
	...
	private void writeFeatureSegment(Writer writer, Feature feature, 
			String geneName, String locusTag,
			Location location, int parentID, int ID) throws IOException {
	...
	private Long writeContig(Writer writer, Location location, int ID, Long contigPosition) throws IOException {
		...
		if (location instanceof Gap) {
			...
			GFF3Writer.writeAttribute(writer, "Name",
					((RemoteLocation)location).getAccession() + "." +
					((RemoteLocation)location).getVersion());
			GFF3Writer.writeAttribute(writer, "Range",
	...
	private void writeAssembly(Writer writer, Assembly assembly, int ID) throws IOException {		
		...
		writer.write(";");
		GFF3Writer.writeAttribute(writer, "Name",
				assembly.getPrimarySpan().getAccession() + "." +
				assembly.getPrimarySpan().getVersion());
		writer.write(";");
		GFF3Writer.writeAttribute(writer, "Range",
