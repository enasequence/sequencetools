...
public class TemplateLoader {
    ...
    protected TemplateSet loadTemplatesFromClassPath(List<String> template_class_paths) throws TemplateException {
    ...
    private void loadTemplateFromClassPath(String templatePath,
                                           TemplateSet results) throws TemplateException {
    ...
    private void addTemplate(TemplateSet results,
                             MutableTemplateInfo template) throws TemplateException {
    ...
    protected Map<String, Integer> parseTemplateOrderFile(InputStream stream) throws TemplateException {
        ...
        try {
            if (stream != null) {//just bail if it is null - can go without ordering
                ...
                /**
                 * this will fail with an index out of bounds exception if the required elements are not present -
                 * just let it fail at runtime as this is loaded at server startup and will get picked up. The order
                 * is determined by the order of the templates in the file.
                 */
                int rowNumber = 0;
    ...
    private void loadprototypeTemplates(String templateDir, TemplateSet results) throws TemplateException {
    ...
    private void loadArchivedTemplates(String templateDir, TemplateSet results) throws TemplateException {
    ...
    private void addTemplatesInDir(File file,
                                   TemplateSet results) throws TemplateException {
        try {
            for (String templateFileName : file.list(new TemplateFileFilter())) {
                FileInputStream stream = new FileInputStream(file.getPath() + File.separator + templateFileName);
                MutableTemplateInfo template = parsseTemplate(stream);
        ...
        for (TemplateTokenInfo tokenInfo : templateInfo.tokenInfos) {
            if (allTokenNames.contains(tokenInfo.getName())) {
                throw new TemplateException("Token name '"
                        + tokenInfo.getName() + "' duplicated in template '" +
                        templateInfo.name + "'.");
            } else {
     ...
     *
     * @param template -
     * @throws uk.ac.ebi.embl.template.exceptions.TemplateException -
     */
    ...
    private void setTokenAndGroupOrder(MutableTemplateInfo templateInfo) {
        ...
        /**
         * for the tokens, first set the order for all tokens based on that which they are declared in the file
         */
        int tokenOrder = 0;
        for (TemplateTokenInfo tokenInfo : templateInfo.tokenInfos) {
        ...
        for (TemplateTokenGroupInfo groupInfo : groupInfos) {
            ...
            /**
             * take any tokens defined in a group and reset their groupOrder to that as defined in the token group
             */
            tokenOrder = 0;
            for (String tokenName : groupInfo.getContainsString()) {
                TemplateTokenInfo tokenInfo = getToken(templateInfo.tokenInfos, tokenName);
                if (tokenInfo == null) {
                    LOGGER.warn("Token " + tokenName + " defined in token group " + groupInfo.getName() + " is not defined in template " + templateInfo.name);
                    continue;
                }
