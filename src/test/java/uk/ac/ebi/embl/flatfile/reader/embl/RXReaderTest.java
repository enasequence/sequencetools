...
public class RXReaderTest extends EmblReaderTest {
  ...
  public void testRead_XrefWithSecondaryAccession() throws IOException {
    ...
    assertEquals("database", publication.getXRefs().get(0).getDatabase());
    assertEquals("primary accession", publication.getXRefs().get(0).getPrimaryAccession());
    assertEquals("secondary accession", publication.getXRefs().get(0).getSecondaryAccession());
    assertEquals("database2", publication.getXRefs().get(1).getDatabase());
    assertEquals("primary accession2", publication.getXRefs().get(1).getPrimaryAccession());
    assertEquals("secondary accession2", publication.getXRefs().get(1).getSecondaryAccession());
  }

  public void testRead_XrefWithoutSecondaryAccession() throws IOException {
    initLineReader("RX    database ; primary accession ");
    Publication publication = lineReader.getCache().getPublication();
    ValidationResult result = (new RXReader(lineReader)).read(entry);
    assertEquals(0, result.count(Severity.ERROR));
    ...
    assertEquals("database", publication.getXRefs().get(0).getDatabase());
    assertEquals("primary accession", publication.getXRefs().get(0).getPrimaryAccession());
    assertNull(publication.getXRefs().get(0).getSecondaryAccession());
  }

  public void testRead_XrefNoDatabase() throws IOException {
    initLineReader("RX    ; primary accession ; secondary  accession .");
    Publication publication = lineReader.getCache().getPublication();
    ValidationResult result = (new RXReader(lineReader)).read(entry);
  ...
  public void testRead_XrefNoPrimaryAccession() throws IOException {
    initLineReader("RX    database ;  ; secondary  accession .");
    Publication publication = lineReader.getCache().getPublication();
    ValidationResult result = (new RXReader(lineReader)).read(entry);
    assertEquals(1, result.count("FF.1", Severity.ERROR));
    assertEquals(0, publication.getXRefs().size());
  ...
  public void testRead_DOI() throws IOException {
    initLineReader("RX    DOI; 10.1007/s00284-007-9029-0.");
    Publication publication = lineReader.getCache().getPublication();
    ValidationResult result = (new RXReader(lineReader)).read(entry);
    assertEquals(0, result.count(Severity.ERROR));
    assertEquals(1, publication.getXRefs().size());
    assertEquals("DOI", publication.getXRefs().get(0).getDatabase());
    assertEquals("10.1007/s00284-007-9029-0", publication.getXRefs().get(0).getPrimaryAccession());
    assertNull(publication.getXRefs().get(0).getSecondaryAccession());
  }
