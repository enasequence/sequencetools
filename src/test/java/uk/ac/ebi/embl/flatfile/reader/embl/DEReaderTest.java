...
public class DEReaderTest extends EmblReaderTest {

  public void testRead_ShortDescription() throws IOException {
    initLineReader("DE     description   description description ");
    ValidationResult result = (new DEReader(lineReader)).read(entry);
    assertEquals("description description description", entry.getDescription().getText());
  ...
  public void testRead_LongDescription() throws IOException {
    initLineReader(
        "DE     description description description \n"
            ...
            + "DE     description description\n"
            + "DE             description description description description description description description description description description description description \n");
    ValidationResult result = (new DEReader(lineReader)).read(entry);
    assertEquals(
  ...
  public void testRead_EmptyLine() throws IOException {
    ...
    initLineReader("DE   ");
    result = (new DEReader(lineReader)).read(entry);
    assertEquals(0, result.count(Severity.ERROR));
    assertNull(entry.getDescription().getText());
    initLineReader("DE         ");
    result = (new DEReader(lineReader)).read(entry);
