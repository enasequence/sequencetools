...
public class TPA_dataclass_FixTest {
  ...
  public void testCheck_DEwithoutTPAKeywords() {
    Entry entry = entryFactory.createEntry();
    entry.setDescription(new Text(" Sequence 1 from Patent sdf EP20080846682."));
    ValidationResult result = check.check(entry);
    assertTrue(result.isValid());
    assertEquals(0, result.getMessages().size());
  }
  ...
  public void testCheck_DEwithTPAKeywords() {
    ...
    entry.setDescription(
        new Text("TPA_reasm: Sequence of 16S-23S rRNA spacer region (ITS1) of strain 439"));
    ValidationResult result = check.check(entry);
    assertTrue(result.isValid());
    assertEquals(1, result.getMessages().size());

    Collection<ValidationMessage<Origin>> messages =
        result.getMessages("TPA_dataclass_Fix", Severity.FIX);
  ...
  public void testCheck_DEwithValidDescription() {
    Entry entry = entryFactory.createEntry();
    entry.setDescription(
        new Text(" TPA: Sequence of 16S-23S rRNA spacer region (ITS1) of strain 439"));
    ValidationResult result = check.check(entry);
    assertTrue(result.isValid());
    assertEquals(0, result.getMessages().size());
  }
