...
public class DeprecatedQualifiersCheckTest {
  ...
  public void setUp() {
    ...
    GlobalDataSets.addTestDataSet(
        GlobalDataSetFile.DEPRECATED_QUALIFIERS,
        new DataRow("partial", "(null)", "N"),
        new DataRow("specific_host", "host", "Y"),
        new DataRow("david", "bod", "N"));
  ...
  public void testCheck_Message() {
    ...
    Collection<ValidationMessage<Origin>> messages =
        result.getMessages("DeprecatedQualifiersCheck", Severity.ERROR);
    assertEquals(
        "The \"partial\" qualifier is deprecated.", messages.iterator().next().getMessage());
  }
}
