...
public class SequenceBasesFixTest {
  ...
  public void testCheck_sequenceWithNsAtBeginAndEnd() {
    ...
    Sequence newsequence =
        sequenceFactory.createSequenceByte("nnnnnnnnabcdefghijklmnopqrstuvwxyznnnnnn".getBytes());
    ...
    Collection<ValidationMessage<Origin>> messages =
        validationResult.getMessages("SequenceBasesFix_2", Severity.FIX);
  ...
  public void testCheck_sequenceWithNsAtEnd() {
    Entry entry = entryFactory.createEntry();
    Sequence newsequence =
        sequenceFactory.createSequenceByte("abcdefghijklmnopqrstuvwxyznnnnnn".getBytes());
    newsequence.setTopology(Sequence.Topology.LINEAR);
    entry.setSequence(newsequence);
    ValidationResult validationResult = check.check(entry);
    assertTrue(validationResult.isValid());
  ...
  public void testCheck_sequenceWithAllNs() {
    Entry entry = entryFactory.createEntry();
    Sequence newsequence =
        sequenceFactory.createSequenceByte("nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn".getBytes());
    newsequence.setTopology(Sequence.Topology.LINEAR);
    entry.setSequence(newsequence);
    ValidationResult validationResult = check.check(entry);
    Collection<ValidationMessage<Origin>> messages =
