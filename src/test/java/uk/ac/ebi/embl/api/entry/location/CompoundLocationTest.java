...
public class CompoundLocationTest {
  ...
  public void testAddLocation_RemoteRange() {
    CompoundLocation<RemoteRange> location = new CompoundLocation<RemoteRange>() {};
    RemoteRange range = new LocationFactory().createRemoteRange("x", 1, 1L, 3L);
  ...
  public void testAddLocations_RemoteRange() {
    CompoundLocation<RemoteRange> location = new CompoundLocation<RemoteRange>() {};
    RemoteRange range = new LocationFactory().createRemoteRange("x", 1, 1L, 3L);
    Collection<RemoteRange> ranges = new ArrayList<RemoteRange>();
    ranges.add(range);
  ...
  public void testGetLength() {
    RemoteRange range = new LocationFactory().createRemoteRange("x", 1, 1L, 3L);
    RemoteRange range2 = new LocationFactory().createRemoteRange("x", 1, 1L, 3L);
    Collection<RemoteRange> ranges = new ArrayList<RemoteRange>();
    ranges.add(range);
    ranges.add(range2);
    CompoundLocation<RemoteRange> location = new CompoundLocation<RemoteRange>() {};
