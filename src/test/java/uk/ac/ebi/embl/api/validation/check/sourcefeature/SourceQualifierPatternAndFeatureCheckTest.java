...
public class SourceQualifierPatternAndFeatureCheckTest {
  ...
  public void setUp() {
    ...
    GlobalDataSets.addTestDataSet(
        GlobalDataSetFile.SOURCE_QUALIFIER_PATTERN_FEATURE,
        new DataRow("LTR", "note", ".*endogenous retrovirus$"));
  ...
  public void testCheck_Valid() {
    source.addQualifier(qualifierFactory.createQualifier("note", "X endogenous retrovirus"));
    entry.addFeature(featureFactory.createFeature("LTR"));
  ...
  public void testCheck_Message() {
    ...
    Collection<ValidationMessage<Origin>> messages =
        result.getMessages("SourceQualifierPatternAndFeatureCheck", Severity.ERROR);
    assertEquals(
