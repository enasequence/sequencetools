...
public class ExclusiveQualifiersCheckTest {
	...
	public void testCheck_pseudoQualifier() {
		...
		Collection<ValidationMessage<Origin>> messages = result.getMessages(
                "ExclusiveQualifiersCheck2", Severity.WARNING);
		assertEquals(1, messages.size());
		assertEquals("Feature annotated with pseudo should not contain a product qualifier, please add the product qualifier value to a note qualifier or add a comment for curator.",
	...
	public void testCheck_Message() {
		...
		ValidationResult result = check.check(feature);
		Collection<ValidationMessage<Origin>> messages = result.getMessages(
                "ExclusiveQualifiersCheck1", Severity.ERROR);
		assertEquals("Qualifiers proviral and virion cannot exist together.",
				messages.iterator().next().getMessage());
	}	
