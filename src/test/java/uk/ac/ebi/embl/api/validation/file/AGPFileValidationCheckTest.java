...
public class AGPFileValidationCheckTest extends SubmissionValidationTest {
  ...
  public void testGenomeSubmissionwithFlatfileAGP()
    ...
    options.reportDir =
        Optional.of(initSubmissionTestFile(fileName, FileType.AGP).getFile().getParent());
    options.processDir =
        Optional.of(initSubmissionTestFile(fileName, FileType.AGP).getFile().getParent());
    ...
    try {
      ...
      sharedInfo.contigDB =
          DBMaker.fileDB(options.reportDir.get() + File.separator + ".contig")
              .closeOnJvmShutdown()
              .fileDeleteAfterOpen()
              .transactionEnable()
              .make();
  ...
  public void testGenomeSubmissionwithFastafileAGP()
    ...
    options.submissionFiles = Optional.of(submissionFiles);
    options.reportDir =
        Optional.of(initSubmissionTestFile(agpFileName, FileType.AGP).getFile().getParent());
    options.processDir =
        Optional.of(initSubmissionTestFile(agpFileName, FileType.AGP).getFile().getParent());
    options.init();
    AGPFileValidationCheck check = new AGPFileValidationCheck(options, sharedInfo);
  ...
  private void validateContig(String contigFileName, FileType fileType)
    ...
    if (fileType == FileType.FASTA) check = new FastaFileValidationCheck(options, sharedInfo);
    if (fileType == FileType.FLATFILE) check = new FlatfileFileValidationCheck(options, sharedInfo);
    check.check(file);
