...
public class QualifierValueRequiredQualifierValueEntryCheckTest {
  ...
  public void setUp() {
    ...
    GlobalDataSets.addTestDataSet(
        GlobalDataSetFile.QUALIFIER_VAL_REQUIRED_QUALIFIER_ENTRY,
        new DataRow("organelle", "mitochondrion", "gene", "12S rRNA"));
  ...
  public void testCheck_NoOrganelleValue() {
    ...
    ValidationResult result = check.check(entry);
    assertEquals(
        1, result.count("QualifierValueRequiredQualifierValueEntryCheck", Severity.WARNING));
  }
  ...
  public void testCheck_Message() {
    ...
    Collection<ValidationMessage<Origin>> messages =
        result.getMessages("QualifierValueRequiredQualifierValueEntryCheck", Severity.WARNING);
    assertEquals(
