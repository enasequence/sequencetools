...
public class FeatureLengthCheckTest {
  ...
  public void testCheck_invalidLocation() {
    ...
    Collection<ValidationMessage<Origin>> intronmessages =
        intronResult.getMessages("FeatureLengthCheck-1", Severity.ERROR);
    assertEquals(
        "\"intron\" usually expected to be at least \"10\" nt long. Please check the accuracy.",
        intronmessages.iterator().next().getMessage());
    ValidationResult exonResult = check.check(exonFeature);
    assertEquals(1, exonResult.count("FeatureLengthCheck-1", Severity.WARNING));
    Collection<ValidationMessage<Origin>> exonmessages =
        exonResult.getMessages("FeatureLengthCheck-1", Severity.WARNING);
    assertEquals(
        "\"exon\" usually expected to be at least \"15\" nt long. Please check the accuracy.",
        exonmessages.iterator().next().getMessage());
  }
