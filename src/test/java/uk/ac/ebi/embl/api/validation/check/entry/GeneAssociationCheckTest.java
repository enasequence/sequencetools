...
public class GeneAssociationCheckTest {
    ...
    public void testCheck_DifferentLocusAssociation() {
        ...
        ValidationResult result = check.check(entry);
        assertTrue(result.isValid());
        assertEquals(1, result.count(GeneAssociationCheck.MESSAGE_ID_DIFFERENT_LOCUS_VALUES, Severity.WARNING));
    }
    ...
    public void testCheck_DifferentLocusAssociationMessage() {
        ...
        assertTrue(result.isValid());
        assertEquals(1, result.count(GeneAssociationCheck.MESSAGE_ID_DIFFERENT_LOCUS_VALUES, Severity.WARNING));
        assertEquals("Features sharing gene \"gene1\" are associated with \"\\locus_tag\" qualifiers with different values (\"bod\" and \"cod\")\".",
                result.getMessages(GeneAssociationCheck.MESSAGE_ID_DIFFERENT_LOCUS_VALUES, Severity.WARNING).iterator().next().getMessage());
    }
    ...
    public void testCheck_DifferentPseudoAssociationMessage() {
        ...
        assertTrue(result.isValid());
        assertEquals(1, result.count(GeneAssociationCheck.MESSAGE_ID_DIFFERENT_LOCUS_VALUES, Severity.WARNING));
        assertEquals("Features sharing gene \"gene1\" are associated with \"\\pseudogene\" qualifiers with different values (\"bod\" and \"cod\")\".",
                result.getMessages(GeneAssociationCheck.MESSAGE_ID_DIFFERENT_LOCUS_VALUES, Severity.WARNING).iterator().next().getMessage());
    }
