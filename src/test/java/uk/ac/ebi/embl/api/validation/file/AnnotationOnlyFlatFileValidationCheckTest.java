...
public class AnnotationOnlyFlatFileValidationCheckTest extends SubmissionValidationTest {
  ...
  public void testAnnotationOnlyFlatFile() throws ValidationEngineException {
    ...
    SubmissionFile file =
        initSubmissionFixedTestFile(
            "valid_AnnotationOnlyFlatfile.txt", SubmissionFile.FileType.ANNOTATION_ONLY_FLATFILE);
    ...
    DB db =
        DBMaker.fileDB(options.reportDir.get() + File.separator + ".sequence3")
  ...
  public void testAnnotationOnlyFlatFilemissingSequence() throws ValidationEngineException {
    ...
    SubmissionFile file =
        initSubmissionFixedTestFile(
            "invalid_AnnotationOnlyFlatfile.txt", SubmissionFile.FileType.FLATFILE);
    SubmissionFiles submissionFiles = new SubmissionFiles();
    submissionFiles.addFile(file);
    options.reportDir = Optional.of(file.getFile().getParent());
    options.processDir = Optional.of(file.getFile().getParent());
    sharedInfo.hasAnnotationOnlyFlatfile = true;
    DB db =
        DBMaker.fileDB(options.reportDir.get() + File.separator + ".sequence2")
            .deleteFilesAfterClose()
            .closeOnJvmShutdown()
            .transactionEnable()
            .make();
  ...
  private void validateInvalidContig(String contigFileName, FileType fileType, DB db)
  ...
  private void validateContig(String contigFileName, FileType fileType, DB db)
      throws ValidationEngineException {
    SubmissionFile file = initSubmissionFixedTestFile(contigFileName, fileType);
    SubmissionFiles submissionFiles = new SubmissionFiles();
    submissionFiles.addFile(file);
