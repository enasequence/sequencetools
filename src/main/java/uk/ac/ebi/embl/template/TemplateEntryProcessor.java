...
public class TemplateEntryProcessor {
  ...
  protected TemplateProcessorResultSet processEntry(
    ...
    if (!checkMandatoryFieldsArePresent(
        templateInfo, templateVariables, templateProcessorResultSet))
      return templateProcessorResultSet;
    if (!checkSelectedHeadersHaveValuesAndAreSupported(
        templateVariables, templateProcessorResultSet)) return templateProcessorResultSet;
    ...
    new SectionExtractor()
        .removeSections(template, this.templateInfo.getSections(), templateVariables);
    ...
    BufferedReader stringReader =
        new BufferedReader(new StringReader(template.toString().trim().concat("\n//")));
  ...
  private boolean checkMandatoryFieldsArePresent(
    ...
    for (String field : mandatoryFieldsList) {
      if (!fieldsMap.containsKey(field)) missingfields += field + ",";
    }
    if (!missingfields.isEmpty()) {
      ValidationMessage<Origin> message =
          new ValidationMessage<Origin>(
              Severity.ERROR,
              "MandatoryFieldsCheck",
              missingfields.substring(0, missingfields.length() - 1));
      templateProcessorResultSet
          .getValidationResult()
          .append(new ValidationResult().append(message));
      return false;
    }
    return true;
  }
  ...
  private boolean checkSelectedHeadersHaveValuesAndAreSupported(
    ...
    for (String header : fieldsMap.keySet()) {
      ...
      if (!templateTokenMap.containsKey(header)) unsupportedHeaders += header + ",";
    ...
    if (!unsupportedHeaders.isEmpty()) {
      ValidationMessage<Origin> message =
          new ValidationMessage<Origin>(
              Severity.ERROR,
              "HeadersSupportedCheck",
              unsupportedHeaders.substring(0, unsupportedHeaders.length() - 1));
      templateProcessorResultSet
          .getValidationResult()
          .append(new ValidationResult().append(message));
      return false;
    ...
    if (!missingValue.isEmpty()) {
      ValidationMessage<Origin> message =
          new ValidationMessage<Origin>(
              Severity.ERROR,
              "HeadersValuesCheck",
              missingValue.substring(0, missingValue.length() - 1));
      templateProcessorResultSet
          .getValidationResult()
          .append(new ValidationResult().append(message));
      return false;
      ...
      if (tokenName.equalsIgnoreCase(TemplateProcessorConstants.ORGANELLE_TOKEN)) {
        template =
            new StringBuilder(
                template
                    .toString()
                    .replace(TemplateProcessorConstants.PP_ORGANELLE_TOKEN, tokenValue));
        return;
      }
    }
    template =
        new StringBuilder(
            template.toString().replace(TemplateProcessorConstants.PP_ORGANELLE_TOKEN, ""));
  }
    ...
    for (String fieldName : templateVariables.getTokenNames()) {
      if (isOrganismField(fieldName)) {
        ...
        if (Utils.isValidTaxId(fieldValue)) {
          ...
          List<Taxon> submittableTaxonList =
              taxonList.stream()
                  .filter(taxon -> taxon.isSubmittable())
                  .collect(Collectors.toList());
          scientificName =
              !submittableTaxonList.isEmpty()
                  ? submittableTaxonList.get(0).getScientificName()
                  : "";
        }
        ...
        if (StringUtils.isEmpty(scientificName)) {
          ValidationMessage<Origin> message =
              new ValidationMessage<Origin>(Severity.ERROR, "MasterEntrySourceCheck_2", fieldValue);
          templateProcessorResultSet
              .getValidationResult()
              .append(new ValidationResult().append(message));
        } else {
          replaceOrganismTokenWithScientificname(
              TemplateProcessorConstants.ORGANISM_TOKEN, scientificName);
          replaceOrganismTokenWithScientificname(
              TemplateProcessorConstants.ORGANISM_NAME_TOKEN, scientificName);
        }
      }
    }
  }
  ...
  public void replacePPGene(TemplateVariables templateVariables) {
    if (!template.toString().contains(TemplateProcessorConstants.PP_GENE_TOKEN)) return;
      ...
      if (markerE.getMarker().equals(marker)) {
        template =
            new StringBuilder(
                template
                    .toString()
                    .replace(TemplateProcessorConstants.PP_GENE_TOKEN, markerE.name()));
        return;
      }
    }
    for (TemplateProcessorConstants.MarkerE markerE : TemplateProcessorConstants.MarkerE.values()) {
        ...
        template =
            new StringBuilder(
                template
                    .toString()
                    .replace(TemplateProcessorConstants.PP_GENE_TOKEN, markerE.name()));
        templateVariables.setTokenValue(
            TemplateProcessorConstants.MARKER_TOKEN, markerE.getMarker());
        return;
      }
    }
  }

  private void addDataToEntry(Entry entry, TemplateVariables templateVariables)
    ...
    try {
      ...
      for (String tokenName : templateVariables.getTokenNames()) {
        ...
        if (tokenName
                .toUpperCase()
                .equals(TemplateProcessorConstants.moloculeTypeE.NCRMOLTYPE.name())
            || tokenName
                .toUpperCase()
                .equals(TemplateProcessorConstants.moloculeTypeE.VMOLTYPE.name())) {
          molType = tokenValue;
          break;
        }
      }
  ...
  private void addSequenceLengthToken(TemplateVariables variables) {
    if (variables.containsToken(TemplateProcessorConstants.SEQUENCE_TOKEN)) {
      int sequenceLength =
          variables.getTokenValue(TemplateProcessorConstants.SEQUENCE_TOKEN).length();
      variables.addToken(
          TemplateProcessorConstants.SEQUENCE_LENGTH_TOKEN, Integer.toString(sequenceLength));
    }
  }
  ...
  private void replaceTokens(TemplateVariables tokens) throws TemplateException {
    try {
      ...
      for (String tokenName : tokens.getTokenNames()) {
        ...
        if (tokenName.equals(TemplateProcessorConstants.SEQUENCE_TOKEN)
            || tokenName.equals(TemplateProcessorConstants.COMMENTS_TOKEN)) continue;
  ...
  private void validateSediment(
      TemplateProcessorResultSet templateProcessorResultSet, TemplateVariables templateVariables) {
    if (templateVariables.getVariables().containsKey(TemplateProcessorConstants.SEDIMENT_TOKEN)) {
      String sediment =
          templateVariables.getVariables().get(TemplateProcessorConstants.SEDIMENT_TOKEN);
      for (TemplateProcessorConstants.SedimentE sedimentE :
          TemplateProcessorConstants.SedimentE.values()) {
        if (sedimentE.getSediment().equals(sediment)) return;
      }
      String values = "";
      for (TemplateProcessorConstants.SedimentE sedimentE :
          TemplateProcessorConstants.SedimentE.values()) values += sedimentE.getSediment() + ",";
      ValidationMessage<Origin> message =
          new ValidationMessage<Origin>(
              Severity.ERROR, "SedimentCheck", sediment, StringUtils.chompLast(values, ","));
      templateProcessorResultSet
          .getValidationResult()
          .append(new ValidationResult().append(message));
    }
  ...
  private void validateMarker(
    ...
    if (templateVariables.getVariables().containsKey(TemplateProcessorConstants.MARKER_TOKEN)) {
      ...
      for (TemplateProcessorConstants.MarkerE markerE :
          TemplateProcessorConstants.MarkerE.values()) {
        if (markerE.getMarker().equals(marker)) return;
      }
      String values = "";
      for (TemplateProcessorConstants.MarkerE markerE : TemplateProcessorConstants.MarkerE.values())
        values += markerE.getMarker() + ",";
      ValidationMessage<Origin> message =
          new ValidationMessage<Origin>(Severity.ERROR, "MarkerCheck", marker, values);
      templateProcessorResultSet
          .getValidationResult()
          .append(new ValidationResult().append(message));
    }
  ...
  /**
   * This method check if the ORGANISM_NAME field value is related to a sample and updates the
  ...
  public Sample validateAndGetSample(TemplateVariables templateVariables) throws Exception {
    ...
    for (String tsvHeader : tsvFieldMap.keySet()) {
      if (isOrganismField(tsvHeader)) {
         ...
         * value is sampleId, bioSampleId, or sample alias. If no sample is returned then the value
         * is organism name.
         */
  ...
  public Sample getSampleById(String sampleId) {
    ...
    if (sample == null) {
      try {
        ...
        SampleRetrievalService sampleRetrievalService =
            SequenceToolsServices.sampleRetrievalService();
