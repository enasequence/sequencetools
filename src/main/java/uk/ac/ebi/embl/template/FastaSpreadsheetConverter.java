...
public class FastaSpreadsheetConverter {
  ...
  public FastaReaderResult convertFASTAFileToSpreadsheet(
      String fastaFilePath, String spreadsheetFilePath, FASTAReaderTokenInfo tokenInfo)
      throws TemplateException {
    this.variableTokens = tokenInfo.getAllSelectedTokens();
    this.constantTokens = tokenInfo.getMegaEntryConstants();
    this.convertFastaMode =
        true; // will be picked up in the fasta reading and also write an xl line
  ...
  public FastaReaderResult readFASTAFile(String fastaFilePath, FASTAReaderTokenInfo tokenInfo)
      throws TemplateException {
    ...
    while (currentLine != null) {
      ...
      if (currentLine.startsWith(FASTA_HEADER_TOKEN)) {
        ...
        if (sequenceNumber != 0) {
          ...
          /**
           * if we get to more than the entry limit - stop saving (but still convert to spreadsheet
           * if set to)
           */
          if (sequenceNumber <= TemplateProcessorConstants.TEMPLATE_MEGA_ENTRY_SIZE) {
            // leaving the sequence number despite being 1 ahead - we count the sequences from 1 in
            // the templates system
            importedTemplateVariables.addEntryValues(sequenceNumber, currentTokenVals);
          }

          if (convertFastaMode) {
            this.csvWriter.writeSpreadsheetVariableRow(
                variableTokens, sequenceNumber, currentTokenVals);
          }
        ...
        if (!sequinFasta) {
          ...
          fastaHeaderParser.processFastaHeaderWithOrders(
              tokenOrders, currentLine, currentTokenVals);
        } else {
          Map<String, TemplateToken> variableTokenDisplayNames =
              tokenInfo.getVariableTokenDisplayNames();
          fastaHeaderParser.processFastaHeaderWithKeyValuePairs(
              variableTokenDisplayNames, currentLine, currentTokenVals);
        }
