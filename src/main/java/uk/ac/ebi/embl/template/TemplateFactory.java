...
public class TemplateFactory {
    ...
    private static final Map<String, List<TemplatePreProcessor>> preProcessors = new HashMap<String, List<TemplatePreProcessor>>();
    private static final TemplatePreProcessor ITS_TEMPLATE_PRE_PROCESSOR = new ITSTemplatePreProcessor();
    private static final TemplatePreProcessor PHYLO_MARKER_TEMPLATE_PRE_PROCESSOR = new PhyloMarkerTemplatePreProcessor();
    private static final TemplatePreProcessor ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR = new OrganelleDETemplatePreProcessor();
    private TemplateSet templateSet;

    static {
        preProcessors.put(TemplateIDs.ITS_TEMPLATE_ID, Arrays.asList(ITS_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.PHYLO_MARKER_TEMPLATE_ID, Arrays.asList(PHYLO_MARKER_TEMPLATE_PRE_PROCESSOR, ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));

        preProcessors.put(TemplateIDs.SINGLE_CDS_MRNA_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.SINGLE_CDS_GENOMIC_DNA_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.NCRNA_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.MOBILE_ELEMENT_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.MULTI_GENE_MARKER_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.MULTI_EXON_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.RIBOSOMAL_RNA_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.SATELLITE_DNA_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.PROMOTER_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.IGS_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.GENE_INTRON_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.ISR_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.TSA_UNANNOTATED_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
        preProcessors.put(TemplateIDs.TSA_ANNOTATED_TEMPLATE_ID, Arrays.asList(ORGANELLE_DE_TEMPLATE_PRE_PROCESSOR));
    }
/*
    @Autowired
...
*/
    ...
    public List<TemplateInfo> getAllLatestTemplates() throws TemplateException {
        ...
        for (String templateId : templateSet.getTemplateIds()) {
            ...
            if (templateInfo.isIncluded())
                latestTemplates.add(templateInfo);
        }
    ...
    public List<TemplateInfo> getBarcodeCompliantLatestIncludedTemplates() {
        List<TemplateInfo> barcodeTemplates = getLatestIncludedTemplatesById(TemplateIDs.BARCODE_COMPLIANT_TEMPLATE_IDs);
    ...
    public List<TemplateInfo> getFrequentlyUsedLatestIncludedTemplates() {
        List<TemplateInfo> frequentlyUsedTemplates = getLatestIncludedTemplatesById(TemplateIDs.FREQUENTLY_USED_CHECKLISTS);
        Collections.sort(frequentlyUsedTemplates, new TemplateOrderComparator<TemplateInfo>(templateOrders));
        return frequentlyUsedTemplates;
    }

    public List<TemplateInfo> getMarkerSequenceLatestIncludedTemplates() {
        List<TemplateInfo> markerSequenceTemplates = getLatestIncludedTemplatesById(TemplateIDs.MARKER_SEQUENCE_CHECKLISTS);
        Collections.sort(markerSequenceTemplates, new TemplateOrderComparator<TemplateInfo>(templateOrders));
        return markerSequenceTemplates;
    }

    public List<TemplateInfo> getVirusSpecificLatestIncludedTemplates() {
        List<TemplateInfo> virusSpecificTemplates = getLatestIncludedTemplatesById(TemplateIDs.VIRUS_SPECIFIC_CHECKLISTS);
        Collections.sort(virusSpecificTemplates, new TemplateOrderComparator<TemplateInfo>(templateOrders));
        return virusSpecificTemplates;
    }

    public List<TemplateInfo> getLargeScaleLatestIncludedTemplates() {
        List<TemplateInfo> largeScaleTemplates = getLatestIncludedTemplatesById(TemplateIDs.LARGE_SCALE_DATA_CHECKLISTS);
        Collections.sort(largeScaleTemplates, new TemplateOrderComparator<TemplateInfo>(templateOrders));
    ...
    public List<TemplateInfo> getMimarksCompliantLatestIncludedTemplates() {
        List<TemplateInfo> mimarksTemplates = getLatestIncludedTemplatesById(TemplateIDs.MIMARKS_COMPLIANT_TEMPLATE_IDs);
