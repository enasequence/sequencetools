...
public class FeatureAndSourceQualifierCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    for (DataRow dataRow :
        GlobalDataSets.getDataSet(GlobalDataSetFile.FEATURE_SOURCE_QUALIFIER).getRows()) {
      String expectedQualifierName = dataRow.getString(0);
      String featureName = dataRow.getString(1);
      if (featureName == null
          || expectedQualifierName == null
      ...
      if (sources == null || sources.isEmpty()) {
        ...
        for (Feature source : sources) {
          isQualifierAvailable =
              SequenceEntryUtils.isQualifierAvailable(expectedQualifierName, source);
