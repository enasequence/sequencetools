...
@Description(
    "Feature {0} is required when molecule type is {1}. \"Join\" locations are only permitted in CDS features on mRNA entries when \"ribosomal_slippage\" or \"exception\" qualifiers are present.")
    ...
    if (isNewEntry) {
      if (moleculeType.equals(Sequence.MRNA_MOLTYPE)
        ...
        for (Feature cdsFeature : SequenceEntryUtils.getFeatures(Feature.CDS_FEATURE_NAME, entry)) {
          ...
          if (cdsFeature.getLocations() instanceof Join
              && cdsFeature.getLocations().getLocations().size() > 1) {
            if (!SequenceEntryUtils.isQualifierAvailable(
                    Qualifier.RIBOSOMAL_SLIPPAGE_QUALIFIER_NAME, cdsFeature)
                && !SequenceEntryUtils.isQualifierAvailable(
                    Qualifier.EXCEPTION_QUALIFIER_NAME, cdsFeature)) {
