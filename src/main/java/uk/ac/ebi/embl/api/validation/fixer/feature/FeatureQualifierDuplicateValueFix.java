...
@Description(
    "Removed the qualifier {1}in feature {2},since {0},{1} qualifiers values are duplicated in the same feature"
...
public class FeatureQualifierDuplicateValueFix extends FeatureValidationCheck {
  ...
  private static final String DIFF_QUALIFIER_VALUES_DUPLICATED =
      "FeatureQualifierDuplicateValueFix_1";
  ...
  public ValidationResult check(Feature feature) {
    ...
    for (Qualifier qualifier : feature.getQualifiers(Qualifier.OLD_LOCUS_TAG)) {
    ...
    if (oldLocusTag_Values.size() != 0 && locusTag_Value != null) {
      if (oldLocusTag_Values.contains(locusTag_Value)) {
        feature.removeQualifiersWithValue(Qualifier.OLD_LOCUS_TAG, locusTag_Value);
        reportMessage(
            Severity.FIX,
            feature.getOrigin(),
