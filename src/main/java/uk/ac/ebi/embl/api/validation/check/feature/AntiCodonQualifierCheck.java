...
public class AntiCodonQualifierCheck extends FeatureValidationCheck {
  ...
  private static final String ANTICODON_AMINO_ACID_NOT_MATCH_MESSAGE_ID =
      "AntiCodonQualifierCheck_6";
  ...
  public ValidationResult check(Feature feature) {
    ...
    try {
      ...
      Qualifier antiCodonQualifier = feature.getSingleQualifier(Qualifier.ANTICODON_QUALIFIER_NAME);
      ...
      AnticodonQualifier antiCodon = new AnticodonQualifier(antiCodonQualifier.getValue());
      ...
      if (aminoAcid == null) {
        reportError(antiCodonQualifier.getOrigin(), ANTICODON_AMINO_ACID_MESSAGE_ID);
      }
      if (aminoAcid != null && aminoAcid.getAbbreviation() != null) {
        if (!aminoAcid.getAbbreviation().equals(aminoAcidString)) {
          reportWarning(
              feature.getOrigin(),
              ANTICODON_AMINO_ACID_NOT_MATCH_MESSAGE_ID,
              aminoAcidString,
      ...
      if (start < beginPosition || end > endPosition) {
        reportError(
            antiCodonQualifier.getOrigin(), ANTICODON_LOCATION_RANGE_MESSAGE_ID, start, end);
      }
      if (start > end) {
        reportError(antiCodonQualifier.getOrigin(), ANTICODON_SEGMENT_MESSAGE_ID, start, end);
      }
      if (start == 0) {
        reportError(antiCodonQualifier.getOrigin(), ANTICODON_SEGMENT_START_MESSAGE_ID, start);
      }
      if (anticodonLocationLength != 3) {
        reportError(antiCodonQualifier.getOrigin(), ANTICODON_SEGMENT_END_MESSAGE_ID, 3);
      }
