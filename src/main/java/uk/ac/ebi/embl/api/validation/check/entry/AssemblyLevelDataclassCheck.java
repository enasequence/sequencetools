...
@Description("Invalid ID Line dataclass {0}" + "{0} dataclass allowed only for Master entries")
@ExcludeScope(
    validationScope = {
...
public class AssemblyLevelDataclassCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    if (assemblyLevel == -1
        && ValidationScope.ASSEMBLY_MASTER.equals(
            getEmblEntryValidationPlanProperty().validationScope.get())) {
      ...
      if (entry.getDataClass() != null
          && (Entry.WGS_DATACLASS.equals(entry.getDataClass())
              || Entry.SET_DATACLASS.equals(entry.getDataClass()))) {
