...
/** Validator for Tax_id column values. Rule: String containing only numbers. */
public class TaxIdValidator implements ColumnValidator {

  private static final String COLUMN_NAME = "Tax_id";
  ...
  public ValidationResult validate(String value, int rowNumber, int columnIndex) {
    ...
    if (!value.matches("^[0-9]+$")) {
      ...
      message.setMessage(
          String.format(
              "Row %d, column %d (%s): Invalid value '%s' - must contain only numbers",
