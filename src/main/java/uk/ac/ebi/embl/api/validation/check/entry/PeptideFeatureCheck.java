...
@Description(
    "Peptide feature sharing \\\"gene\\\" or \\\"locus_tag\\\" qualifier with CDS containing \\\"pseudo\\\" qualifier must also contain \\\"pseudo\\\" qualifier.\n"
...
public class PeptideFeatureCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    try {
      ...
      Collection<Feature> cdsFeatures =
          SequenceEntryUtils.getFeatures(Feature.CDS_FEATURE_NAME, entry);
      List<PeptideFeature> peptideFeatures =
          getPeptideFeatures(entry.getFeatures()); // a list of all peptide features
      ...
      if (!peptideFeatures.isEmpty()) {
        ...
        for (Feature cdsFeature : cdsFeatures) {
          ...
          for (Feature peptideFeature : peptideFeatures) {
            ...
            if ((areSame(cdsFeature, peptideFeature, Qualifier.LOCUS_TAG_QUALIFIER_NAME)
                    && areLocationsOnSameStrand(
                        cdsFeature.getLocations(), peptideFeature.getLocations()))
                || (areSame(cdsFeature, peptideFeature, Qualifier.GENE_QUALIFIER_NAME)
                    && doLocationsOverlap(
                        cdsFeature.getLocations(), peptideFeature.getLocations()))) {
    ...
    if (!checkPseudoQualifier(cdsFeature, relevantPeptideFeatures)) {
      ...
      try {
        ...
        for (PeptideFeature peptideFeature : relevantPeptideFeatures) {
          ...
          translator.configureFromFeature(
              peptideFeature, getEmblEntryValidationPlanProperty().taxonClient.get(), entry);

          ExtendedResult<TranslationResult> peptideResult = null;

          if (segment != null) {
            peptideResult =
                translator.translate(segment.getSequenceByte(), peptideFeature.getOrigin());
          }

          /**
           * append the validation results from translating the peptide, unlike the CDS results
           * which we ignore (they will be picked up in the CDS checks.
           */
          result.append(peptideResult);

          /** is the translation length a multiple of 3?... */
          if (peptideResult != null
              && peptideResult.getExtension().getTranslationLength() % 3 != 0) {
          ...
          if (peptideResult != null
              && peptideResult.getExtension() != null
              && cdsValidationResult.getExtension() != null) {
  ...
  /**
   * if cds has pseudo qualifier, throw an error for every peptide feature that does not also have
   * pseudo
   */
  private boolean checkPseudoQualifier(
      CdsFeature cdsFeature, List<PeptideFeature> relevantPeptideFeatures) {
  ...
  /**
   * a simplistic implementation to check for strandedness. This just looks for any case where one
   * strand differs from another and returns false. A more complex implementation would check for
   * whether a fragment of the location on one strand overlaps with a fragment from the other
   * location on the same strand but this one just wants the whole thing to be on the same strand
   * for both locations.
   *
   * @param location1
   * @param location2
   * @return
   */
  private boolean areLocationsOnSameStrand(
  ...
  private boolean areSame(Feature cdsFeature, Feature peptideFeature, String qualifierName) {
    ...
    Qualifier peptideLocusQualifier =
        SequenceEntryUtils.getQualifier(qualifierName, peptideFeature);

    return cdsLocusQualifier != null
        && peptideLocusQualifier != null
