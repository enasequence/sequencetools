...
public class AnticodonQualifierFix extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    ArrayList<Qualifier> anticodonQualifiers =
        (ArrayList<Qualifier>)
            SequenceEntryUtils.getQualifiers(Qualifier.ANTICODON_QUALIFIER_NAME, entry);
    if (anticodonQualifiers.isEmpty()) return result;
      ...
      try {
        ...
        String aminoAcid =
            qualifier.getAminoAcid() != null
                ...
                : null; // valid amino acid(e.g. aa: Sec) from the anticodon (e.g. aa :SeC) value
        String aminoAcidString =
            qualifier.getAminoAcidString(); // from the original anticodon value(e.g. aa :SeC)
        SegmentFactory factory = new SegmentFactory();
        Segment segment =
            (entry.getSequence() != null && entry.getSequence().getSequenceByte() != null)
                ? factory.createSegment(entry.getSequence(), location)
                : null;
        ...
        if (qualifier.getSequence() == null) {
          reportMessage(
              Severity.FIX, anticodonQualifier.getOrigin(), SEQUENCE_FIX_ID, anticodonValue);
        }
  ...
  private void reportException(
      ValidationResult result, ValidationException exception, Qualifier qualifier) {
