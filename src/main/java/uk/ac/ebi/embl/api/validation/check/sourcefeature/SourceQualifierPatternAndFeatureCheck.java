...
public class SourceQualifierPatternAndFeatureCheck extends EntryValidationCheck {
  ...
  private static final String MESSAGE_ID = "SourceQualifierPatternAndFeatureCheck";
  ...
  public ValidationResult check(Entry entry) {
    ...
    for (DataRow dataRow :
        GlobalDataSets.getDataSet(GlobalDataSetFile.SOURCE_QUALIFIER_PATTERN_FEATURE).getRows()) {
            ...
            if (value != null) {
              if (pattern.matcher(value).matches()) {
                if (!SequenceEntryUtils.isFeatureAvailable(featureName, entry)) {
                  reportError(
                      entry.getOrigin(), MESSAGE_ID, featureName, qualifierName, valuePattern);
                }
              }
            }
          }
