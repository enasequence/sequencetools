...
@Description(
    "Added source qualifier {0} as qualifier {2} value matches the pattern {3}"
...
public class SourceQualifierMissingFix extends EntryValidationCheck {
  ...
  private static final String addEnvironmentalSampleForUnculturedOrg =
      "SourceQualifierMissingFix_1";
  private static final String addEnvironmentalSampleForMetagenomeOrg =
      "SourceQualifierMissingFix_2";
  ...
  public ValidationResult check(Entry entry) {
    ...
    if (entry.getPrimarySourceFeature().getTaxId()
        != null) // set the scientific name based on taxid
    {
      Taxon taxon =
          getEmblEntryValidationPlanProperty()
              .taxonClient
              .get()
              .getTaxonByTaxid(entry.getPrimarySourceFeature().getTaxId());
      if (taxon != null)
    ...
    boolean is_environment_sample_exists =
        entry
                .getPrimarySourceFeature()
                .getQualifiers(Qualifier.ENVIRONMENTAL_SAMPLE_QUALIFIER_NAME)
                .size()
            != 0;
    boolean is_isolation_source_exists =
        entry
                .getPrimarySourceFeature()
                .getQualifiers(Qualifier.ISOLATION_SOURCE_QUALIFIER_NAME)
                .size()
            != 0;
    ...
    if (scientificName != null) {
      boolean isSourceOrganismMetagenome =
          getEmblEntryValidationPlanProperty() != null
              && getEmblEntryValidationPlanProperty()
                  .taxonClient
                  .get()
                  .isOrganismMetagenome(scientificName);

      boolean hasMetagenomeSource =
          fixMetagenomeSource(entry.getPrimarySourceFeature(), scientificName);
      ...
      if (pattern.matcher(scientificName).matches() && !is_environment_sample_exists) {
        ...
        isolation_source_value =
            isolation_source_value == null || isolation_source_value.isEmpty()
                ? "unknown"
                : isolation_source_value.trim();

        entry
            .getPrimarySourceFeature()
            .addQualifier(Qualifier.ISOLATION_SOURCE_QUALIFIER_NAME, isolation_source_value);
      ...
      if (!is_environment_sample_exists) {
        Taxon taxon =
            getEmblEntryValidationPlanProperty()
                .taxonClient
                .get()
                .getTaxonByScientificName(scientificName);
        ...
        if (taxon != null && taxon.getLineage() != null) {
          ...
          if (lineage.contains("environmental samples")) {
            entry
                .getPrimarySourceFeature()
                .addQualifier(Qualifier.ENVIRONMENTAL_SAMPLE_QUALIFIER_NAME);
            is_environment_sample_exists = true;
            reportMessage(
                Severity.FIX,
                entry.getPrimarySourceFeature().getOrigin(),
                lineageEnvironmentQualifierFix_ID);
          }
        }
      }
    }
    ...
    if (!is_isolation_source_exists && is_environment_sample_exists) {
      entry
          .getPrimarySourceFeature()
          .addQualifier(Qualifier.ISOLATION_SOURCE_QUALIFIER_NAME, "unknown");
      reportMessage(
          Severity.FIX,
          entry.getPrimarySourceFeature().getOrigin(),
          unculturedOrgIsolationQualifierFix_ID,
          "unknown");
    }

    if (is_environment_sample_exists
        && entry.getPrimarySourceFeature().getQualifiers(Qualifier.STRAIN_QUALIFIER_NAME).size()
            != 0) {
      entry.getPrimarySourceFeature().removeQualifier(Qualifier.STRAIN_QUALIFIER_NAME);
      if (entry.getPrimarySourceFeature().getQualifiers(Qualifier.ISOLATE_QUALIFIER_NAME).size()
          == 0) {
        ...
        reportMessage(
            Severity.FIX, entry.getPrimarySourceFeature().getOrigin(), strainToIsolateFix);
      }
    }
  ...
  private boolean fixMetagenomeSource(SourceFeature source, String organismScientificName) {
    ...
    List<Qualifier> metagenomeSourceQualifiers =
        ...
        if (getEmblEntryValidationPlanProperty() != null) {
          Taxon taxon =
              getEmblEntryValidationPlanProperty()
                  .taxonClient
                  .get()
                  .getTaxonByScientificName(metagenomeSourceScientificName);
          if (taxon != null) {
            if (taxon.getScientificName().equalsIgnoreCase(organismScientificName)) {
              ...
              if (!metagenomeSourceScientificName.equals(taxon.getScientificName())) {
                ...
                reportMessage(
                    ...
                    metagenomeSourceScientificName,
