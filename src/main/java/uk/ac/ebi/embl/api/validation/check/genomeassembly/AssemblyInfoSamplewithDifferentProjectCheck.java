...
@Description("")
@RemoteExclude
public class AssemblyInfoSamplewithDifferentProjectCheck
  ...
  public ValidationResult check(AssemblyInfoEntry entry) throws ValidationEngineException {
    ...
    if (getEraproDAOUtils() == null) return result;
    try {
      ValidationResult result = new ValidationResult();
      List<String> analysisIdList =
          getEraproDAOUtils().isSampleHasDifferentProjects(entry.getAnalysisId());
      if (analysisIdList.size() > 0)
        reportError(
            entry.getOrigin(),
            MESSAGE_KEY_DIFFERENT_PROJECT_ASEMBLY_ERROR,
            String.join(",", analysisIdList));
      return result;
