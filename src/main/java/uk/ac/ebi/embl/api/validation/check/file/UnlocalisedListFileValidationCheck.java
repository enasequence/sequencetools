...
public class UnlocalisedListFileValidationCheck extends FileValidationCheck {
  ...
  ValidationResult validateValidChromosomeEntry(UnlocalisedEntry unlocalisedEntry) {
    ...
    if (unlocalisedEntry.getChromosomeName() != null) {
      if (sharedInfo.chromosomeNames.size() != 0
          && !sharedInfo.chromosomeNames.contains(
              unlocalisedEntry.getChromosomeName().toUpperCase())) {
        ValidationMessage message =
            new ValidationMessage<>(
                Severity.ERROR,
                "UnlocalisedListChromosomeValidCheck",
                unlocalisedEntry.getChromosomeName());
  ...
  ValidationResult validateValidUnlocalisedEntry(UnlocalisedEntry unlocalisedEntry) {
    ...
    if (unlocalisedEntry.getObjectName() != null) {
      if (sharedInfo.entryNames.size() != 0
          && !sharedInfo.entryNames.contains(unlocalisedEntry.getObjectName().toUpperCase())) {
        ValidationMessage message =
            new ValidationMessage<>(
                Severity.ERROR,
                "UnlocalisedListUnlocalisedValidCheck",
                unlocalisedEntry.getObjectName());
        result.append(message);
      }
    }
    return result;
