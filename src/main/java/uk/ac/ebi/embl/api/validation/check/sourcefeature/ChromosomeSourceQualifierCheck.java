...
public class ChromosomeSourceQualifierCheck extends FeatureValidationCheck {
  ...
  private static final String CHROMOSOME_SOURCE_QUALIFIER_MISSING_ERROR =
      "ChromosomeSourceQualiferCheck_2";
  ...
  public ValidationResult check(Feature feature) {
    ...
    if (!getEmblEntryValidationPlanProperty()
        .validationScope
        .get()
        .equals(ValidationScope.ASSEMBLY_CHROMOSOME)) {
      return result;
    }
    ...
    if (source.getSingleQualifier(Qualifier.SEGMENT_QUALIFIER_NAME) != null) cnt++;

    if (SequenceEntryUtils.isQualifierWithValueAvailable(
        Qualifier.NOTE_QUALIFIER_NAME, "monopartite", source)) cnt++;
