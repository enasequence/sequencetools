...
@Description(
    "Qualifier \"{0}\" has been removed from Feature \"{1}\" as qualifier \"{0}\" must not exist if taxonomic divison has value {2}.")
public class TaxonomicDivisionNotQualifierFix extends EntryValidationCheck {

  private static final String TAXONOMIC_DIVISION_MESSAGE_ID = "TaxonomicDivisionNotQualifierFix";

  public TaxonomicDivisionNotQualifierFix() {}

  public ValidationResult check(Entry entry) {
    DataSet dataSet = GlobalDataSets.getDataSet(GlobalDataSetFile.TAXONOMIC_DIVISION_NO_QUALIFIER);
    ...
    if (taxon == null || taxon.getDivision() == null) return result;

    List<Feature> sourceFeatures =
        SequenceEntryUtils.getFeatures(Feature.SOURCE_FEATURE_NAME, entry);

    if (sourceFeatures == null || sourceFeatures.size() == 0) return result;
