...
public class ContigstosequenceFix extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    if (entry.getSequence() == null
        || entry.getSequence().getContigs() == null
        || entry.getSequence().getContigs().size() == 0) {
      return result;
    }

    if (getEntryDAOUtils() == null
        || FileType.AGP.equals(getEmblEntryValidationPlanProperty().fileType.get())) return result;
    ...
    try {
      ...
      if (sequenceLength == 0) sequenceLength = (int) entry.getIdLineSequenceLength();
