   ...
   @Override
	public ValidationResult check(AssemblyInfoEntry entry) throws ValidationEngineException
	{
		...
		if(!assemblyNamePattern.matcher(entry.getName()).matches())
			reportError(entry.getOrigin(), MESSAGE_KEY_INVALID_ASEMBLY_NAME_ERROR,entry.getName(),assemblyNameRegex);

		if(entry.getName().length()>=100)
			reportError(entry.getOrigin(), MESSAGE_KEY_INVALID_ASEMBLY_NAME_LENGTH_ERROR,entry.getName());
		return result;
	}
