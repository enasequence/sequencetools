...
public class MoleculeTypeAndQualifierFix extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    if (entry.getPrimarySourceFeature() == null) return result;

    String molType =
        SequenceEntryUtils.getMoleculeType(entry) == null
            ? entry
                .getPrimarySourceFeature()
                .getSingleQualifierValue(Qualifier.MOL_TYPE_QUALIFIER_NAME)
            : SequenceEntryUtils.getMoleculeType(entry);
    ...
    for (DataRow row : dataSet.getRows()) {
      ...
      if (molType.equals(requiredmolType_value)) continue;
      List<Qualifier> qualifiers = SequenceEntryUtils.getQualifiers(qualifier, entry);
      if (qualifiers.size() == 0) continue;
      Collection<Feature> features =
          SequenceEntryUtils.getFeaturesContainingQualifier(qualifier, entry);
