...
@Description(
    "Sequence contains a stretch of 'n' characters between base {0} and {1} that is not "
...
@ExcludeScope(
    ...
    }) // do not run in template mode - no ability to add gaps
public class SequenceToGapFeatureBasesCheck extends EntryValidationCheck {
  ...
  protected static final String MESSAGE_SEQ_WITH_MORE_N = "SequenceWithMoreNs";
  ...
  public ValidationResult check(Entry entry) {
    ...
    gapFeatures.addAll(
        assembly_gapFeatures); // assembly_gaps and gaps are mutually exclusive,there is other check
    // to complain
    ...
    for (NRegion nRegion : nRegions) {
      ...
      for (Feature gapFeature : gapFeatures) {
        ...
        if (compoundLocation == null
          ...
          // if there is more than 1 location, just bail, there are other checks to complain if the
          // location is
