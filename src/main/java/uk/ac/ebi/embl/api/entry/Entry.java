...
public class Entry implements HasOrigin, Serializable, Comparable<Entry> {
  ...
  public enum Status {
    ...
    public static Status getStatus(int statusId) {
      for (Status s : values()) {
        if (statusId == s.getStatusId()) return s;
      }
  ...
  public Entry() {
    ...
    this.projectAccessions = new ArrayList<>();
  ...
  public <T extends Feature> List<T> getComplexFeatures(String name, List<T> features) {
    ...
    for (Feature feature : this.features) {
      ...
      if (feature != null && featureName != null && featureName.equals(name)) {
  ...
  public boolean isMaster() {
    return (this.contigAccessions.size() > 0 || this.scaffoldAccessions.size() > 0);
  }
