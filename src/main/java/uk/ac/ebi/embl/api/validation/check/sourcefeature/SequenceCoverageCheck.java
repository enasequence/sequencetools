...
public class SequenceCoverageCheck extends EntryValidationCheck {
    ...
    public ValidationResult check(Entry entry) {
        ...
        if (sources.stream().anyMatch(SourceFeature::isTransgenic)) {
            ...
            if (!validateFullCoverageNoGapsOrOverlaps(transgenicLocations, sequenceLength, firstSourceOrigin)) {
    ...
    private static List<Location> sortedLocations(List<SourceFeature> sources, Predicate<SourceFeature> add) {
    ...
    private void validateWithinSequenceNoOverlaps(List<Location> locations, long sequenceLength, Origin firstSourceOrigin) {
     ...
     */
    private boolean validateFullCoverageNoGapsOrOverlaps(List<Location> locations, long sequenceLength, Origin firstSourceOrigin) {
        ...
        if (!validateEndPositionIsSequenceLength(location, sequenceLength, firstSourceOrigin)) {
            return false;
        }
        return true;
    }
    ...
    private boolean validateStartPositionWithinSequence(Location location, long sequenceLength, Origin firstSourceOrigin) {
    ...
    private boolean validateEndPositionIsSequenceLength(Location location, long sequenceLength, Origin firstSourceOrigin) {
    ...
    private boolean validateNoGapBetweenLocations(Location prevLocation, Location location, Origin firstSourceOrigin) {
    ...
    private boolean validateNoOverlapBetweenLocations(Location prevLocation, Location location, Origin firstSourceOrigin) {
        if ((maxPosition(prevLocation) + 1) > minPosition(location)) {
    ...
    private static long minPosition(Location location) {
        return location.getBeginPosition() <= location.getEndPosition() ? location.getBeginPosition() : location.getEndPosition();
    }
