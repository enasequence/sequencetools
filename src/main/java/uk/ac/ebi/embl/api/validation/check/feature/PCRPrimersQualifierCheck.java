...
@Description(
    "Qualifier \"{0}\" has invalid format"
...
public class PCRPrimersQualifierCheck extends FeatureValidationCheck {
  ...
  public ValidationResult check(Feature feature) {
    ...
    for (DataRow dataRow : GlobalDataSets.getDataSet(GlobalDataSetFile.NUCLEOTIDE_CODE).getRows()) {
       ...
       * cv_nucleotide.code )
       */
      List<Qualifier> pcrQualifiers = feature.getQualifiers(Qualifier.PCR_PRIMERS_QUALIFIER_NAME);
        ...
        for (String pcrToken : pcrTokens) {
          ...
          if (fwd_seq_matcher.find()) {
            ...
            if (!validateNucleotides(fwd_seq, pcrQualifier, dataRow)) {
              reportError(
                  pcrQualifier.getOrigin(),
                  NUCLEOTIDE_MESSAGE_ID_3,
          ...
          if (remove_pattern.matcher(pcrToken).find()) {
            reportError(pcrQualifier.getOrigin(), PCR_FORMAT_MESSAGE_ID, pcrQualifier.getName());
          }
        }
      }
    }
  ...
  private boolean validateNucleotides(String sequence, Qualifier qualifier, DataRow dataRow) {
    ...
    for (char c : nucleotides) {
      ...
      if (c == '<') {
        if (openAngleBracketFound) {
          reportError(
              qualifier.getOrigin(), NUCLEOTIDE_MESSAGE_ID_1, qualifier.getName(), sequence);
        }
    ...
    if (openAngleBracketFound) {
      reportError(qualifier.getOrigin(), NUCLEOTIDE_MESSAGE_ID_1, qualifier.getName(), sequence);
    }
