...
public class OrganismAndPermittedQualifierCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    for (DataRow dataRow : dataSet.getRows()) {
      ...
      if (getEmblEntryValidationPlanProperty().taxonClient.get().isOrganismValid(organism)
          && !getEmblEntryValidationPlanProperty()
              .taxonClient
              .get()
              .isChildOfAny(organism, organisms)) {
        reportError(
            source.getOrigin(),
            MESSAGE_ID_2,
            organism,
            Utils.paramArrayToString(organisms),
            permittedQualifier,
            Utils.paramArrayToString(organisms));
      }
    }
    if (organismQualifier != null
        ...
        && (getEmblEntryValidationPlanProperty()
                .taxonClient
                .get()
                .isChildOf(organismQualifier.getValue(), "Bacteria")
            || getEmblEntryValidationPlanProperty()
                .taxonClient
                .get()
                .isChildOf(organismQualifier.getValue(), "Archaea"))
        && getEmblEntryValidationPlanProperty()
            .validationScope
            .get()
            .equals(ValidationScope.EMBL_TEMPLATE)) {

      List<Qualifier> geneQualifiers =
          SequenceEntryUtils.getQualifiers(Qualifier.GENE_QUALIFIER_NAME, entry);
      List<Qualifier> productQualifiers =
          SequenceEntryUtils.getQualifiers(Qualifier.PRODUCT_QUALIFIER_NAME, entry);
      ...
      if (geneQualifiers.size() != 0 || productQualifiers.size() != 0) {
        ...
        for (Qualifier productQualifier : productQualifiers) {
          ...
          if (productValue != null
              && !productPattern.matcher(productValue.toLowerCase()).matches()) {
            reportWarning(source.getOrigin(), PROKARYOTIC_MESSAGE);
            return result;
          }
          matchProduct = true;
      ...
      if (!matchGene && !matchProduct) reportWarning(source.getOrigin(), PROKARYOTIC_MESSAGE);
    }
  ...
  private void reportError(Feature feature, String[] organisms, String permittedQualifier) {
    reportError(
        feature.getOrigin(), MESSAGE_ID_1, permittedQualifier, Utils.paramArrayToString(organisms));
  }
}
