...
public class LocustagExistsCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    if (FileType.EMBL.equals(getEmblEntryValidationPlanProperty().fileType.get())
      ...
      if (entry.getPrimarySourceFeature() != null) {
        ...
        if (taxonClient.isChildOf(scientificName, "Viruses")) return result;
      }
      if (SequenceEntryUtils.hasAnnotation(entry)
          && !SequenceEntryUtils.isQualifierAvailable(Qualifier.LOCUS_TAG_QUALIFIER_NAME, entry)) {
