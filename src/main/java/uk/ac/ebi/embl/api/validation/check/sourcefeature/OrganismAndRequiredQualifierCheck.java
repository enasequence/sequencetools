...
@Description(
    "At least one of the following qualifiers {0} must exist when organism belongs to {1}.")
@ExcludeScope(
    validationScope = {
      ValidationScope.EPO,
...
public class OrganismAndRequiredQualifierCheck extends FeatureValidationCheck {
  ...
  private void checkFeatureAgainstRow(DataRow dataRow, SourceFeature sourceFeature) {
    ...
    Severity severity =
        Severity.valueOf(
            dataRow.getString(2)); // will throw an illegal argument if it cant recognize
    ...
    Qualifier organismQualifier =
        sourceFeature.getSingleQualifier(Qualifier.ORGANISM_QUALIFIER_NAME);
    ...
    for (String expectedOrganism : expectedOrganisms) {
      if (getEmblEntryValidationPlanProperty()
          .taxonClient
          .get()
          .isChildOf(organism, expectedOrganism)) {
        reportMessage(
            severity,
            sourceFeature.getOrigin(),
            MESSAGE_ID,
            Utils.paramArrayToString(requiredQualifiers),
            expectedOrganism);
      }
    }
  }
