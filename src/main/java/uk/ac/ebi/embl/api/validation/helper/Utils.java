...
public class Utils {
  ...
  private static final Map<String, Character> entityNameToChar =
  ...
  /**
   * Check whether value NOT matches the pattern build with prefix, middle part and post-fixer.
   *
   ...
   */
  public static boolean notMatches(String value, String prefix, String middle, String postfix) {
    return !matches(value, prefix, middle, postfix);
  }

  /**
   * Check whether value matches the pattern build with prefix, middle part and post-fixer.
   *
   * @param value a value to be checked
   * @param prefix a prefix pattern
   * @param middle a middle pattern
   ...
   */
  public static boolean matchesWithoutPrefixes(
      String value1, String prefix1, String value2, String prefix2) {
  ...
  public static FlattenedMessageResult flattenMessages(
      List<ValidationMessage<Origin>> validationMessages, int messageFlattenThreshold) {
      ...
      if (messageCounts.containsKey(messageKey)) {
        ...
        if (count > messageFlattenThreshold) {
          String messageExample =
              ValidationMessageManager.getString(messageKey); // get the unformatted message string
          message.setMessage(
              messageExample
                  + " ("
                  + count
                  + " occurrences)"); // swap the message to contain the example message
          // (unformatted)
   ...
   */
  public static ArrayList<ValidationMessage> shiftLocation(
      Entry entry, int deletedBeginNs, boolean removeall) {
    ...
    for (int i = 0; i < features.size(); i++) {
      ...
      for (Location location : feature.getLocations().getLocations()) {
        ...
        if (location.getBeginPosition() <= deletedBeginNs
          ...
          location.setBeginPosition(location.getBeginPosition() - deletedBeginNs);
          location.setEndPosition(location.getEndPosition() - deletedBeginNs);
          ...
          if (feature.getName().equals(Feature.GAP_FEATURE_NAME)) {
            validationMessages.add(
                ValidationMessage.message(
                    Severity.FIX,
                    UTILS_2,
                    location.getBeginPosition().toString(),
      ...
      if (!invalidFeature) {
        for (Qualifier qualifier : feature.getQualifiers()) {
          ...
          if (qualifier.getName().equals(Qualifier.TRANSL_EXCEPT_QUALIFIER_NAME)) {
            TranslExceptQualifier translExcepttqualifier =
            ...
            /*if (shiftLocationQualifier(translExcepttqualifier,
              ...
              .getName()
              .equals(Qualifier.RPT_UNIT_RANGE_QUALIFIER_NAME)) {

            Rpt_Unit_RangeQualifier rptUnitRangequalifier =
                new Rpt_Unit_RangeQualifier(qualifier.getValue());
            if (shiftLocationQualifier(rptUnitRangequalifier, deletedBeginNs, feature) != null) {
              validationMessages.add(
            ...
            if (shiftLocationQualifier(tagPeptidequalifier, deletedBeginNs, feature) != null) {
              validationMessages.add(
                  shiftLocationQualifier(tagPeptidequalifier, deletedBeginNs, feature));
            }
          }
        }
      }
    ...
    if (invalidFeatures.size() > 0) {
      for (Feature feature : invalidFeatures) {
        ValidationMessage<Origin> message =
            ValidationMessage.message(Severity.FIX, UTILS_5, feature.getName(), feature.getName());
        message.getOrigins().add(feature.getOrigin());
        validationMessages.add(message);
        entry.removeFeature(feature);
      }
      ...
      if (qualifier.getLocation().getBeginPosition() >= deletedBeginNs) {
        ...
        if (!(qualifier.getLocation().getEndPosition() == null)) {
          ...
          if (newBeginLocation >= featureNewBeginLocation
            ...
            newLocation = factory.createLocalRange(newBeginLocation, newEndLocation);
            qualifier.setLocation(newLocation);
          } else {
            return ValidationMessage.message(
                Severity.ERROR, UTILS_3, qualifier.getName(), newBeginLocation, newEndLocation);
          }
        } else {
          if (newBeginLocation > featureNewBeginLocation
  ...
  public static ValidationMessage shiftReferenceLocation(Entry entry, long newSequenceLength) {
      ...
      for (Location rlocation : reference.getLocations().getLocations()) {
        {
          ...
          if (rlocation.getBeginPosition().equals(rlocation.getEndPosition())) {
            return ValidationMessage.message(
                Severity.WARNING,
                UTILS_6,
                rlocation.getBeginPosition(),
  ...
  /**
   * Shifting the feature Locations according to the new sequence locations and remove the features
   * which have been placed fully inside the 'n' start and end.
   *
   * @param entry
   * @param deletedBeginNs (number of deleted 'n's at the beginning of sequence)
   * @return ArrayList (Validation Messages)
  ...
  public static ArrayList<ValidationMessage> shiftAndRemoveFeature(
    ...
    for (int i = 0; i < features.size(); i++) {
      ...
      for (int j = 0; j < feature.getLocations().getLocations().size(); j++) {
        ...
        Location location = feature.getLocations().getLocations().get(j);
        /*
         * check for all feature locations exists in the entry are
         * within range of sequence Begin and End positions and shifting
         * the locations of the feature according to the new sequence
  ...
  public static String getComponentTypeId(Entry contigEntry) {
    ...
    if (Entry.WGS_DATACLASS.equals(contigEntry.getDataClass())) {
      ...
      for (Text keywordtext : keywords) {
        ...
        if (keyword.contains("PHASE1")
            || keyword.contains("PHASE0")
            || keyword.contains("PHASE2")) {
          ...
          if (c == ';') {
            if (entityNameToChar.containsKey(toBeReplaced.toString())) {
              ...
              if (m.matches()) {
                ...
                replaced.append(
                    ascii >= 32 && ascii <= 126 ? Character.toString((char) ascii) : toBeReplaced);
              } else {
                replaced.append(toBeReplaced);
              }
            }
  ...
  public static ValidationResult validateAssemblySequenceCount(
  ...
  public static void setAssemblyLevelDescription(
      String masterDescription, Integer assemblyLevel, Entry entry) {
    if (null == entry.getSubmitterAccession()) {
      ...
      if (Entry.WGS_DATACLASS.equals(entry.getDataClass()))
        entry.setDescription(
            new Text(getContigDescription(masterDescription, entry.getSubmitterAccession())));
      else
        entry.setDescription(
            new Text(getScaffoldDescription(masterDescription, entry.getSubmitterAccession())));

    } else {
      SourceFeature feature = entry.getPrimarySourceFeature();
      String description =
          getChromosomeDescription(feature, masterDescription, entry.getSubmitterAccession());
  ...
  public static String getChromosomeDescription(
      SourceFeature feature, String masterDescription, String submitterAccession) {
    ...
    if (null != plasmid && null != plasmid.getValue()) {
      ...
      List<Qualifier> monopartiteQualifier =
          note.stream()
              .filter(qual -> "monopartite".equals(qual.getValue()))
              .collect(Collectors.toList());
      if (monopartiteQualifier.size() > 0) {
        StringBuilder descr =
            new StringBuilder(String.format("%s, complete genome: ", masterDescription));
        monopartiteQualifier.forEach(
            (q) -> {
              descr.append(q.getValue());
            });
        return descr.toString();
      } else return String.format("%s, %s", masterDescription, submitterAccession);
    } else {
  ...
  private static boolean areContinuous(String prev, String curr) throws IllegalArgumentException {
    ...
    if (mPrev.matches() && mCurr.matches()) {
      ...
      return (mPrev.group(1).equalsIgnoreCase(mCurr.group(1))
          && prevNumStr.length() == currNumStr.length()
          && Integer.parseInt(prevNumStr) + 1 == Integer.parseInt(currNumStr));
    } else {
  ...
  public static List<Text> createRange(List<Text> secondaryAccessions, Origin origin) {
    ...
    if (secondaryAccessions == null || secondaryAccessions.isEmpty()) return accessionRange;
    ...
    if (origin == null) {
      ...
      if (secondaryAccessions.size() > 1 && origin != null) {
        if (origin instanceof FlatFileOrigin) {
          ...
          FlatFileOrigin lastOrigin =
              (FlatFileOrigin) secondaryAccessions.get(secondaryAccessions.size() - 1).getOrigin();
          if (lastOrigin != null) {
            origin =
                new FlatFileOrigin(
                    firstOrigin.getFileId(),
                    firstOrigin.getFirstLineNumber(),
                    lastOrigin.getLastLineNumber());
          }
        }
      }
    }
      ...
      if (currSecAccn.getText().contains("-")) {
        ...
        if (prevAccn == null) {
          ...
          if (areContinuous(prevAccn.getText(), currSecAccn.getText())) {
            if (firstAcc == null) firstAcc = prevAccn.getText();
  ...
  public static List<Text> expandRanges(Text... secondaryAccessions) throws IllegalStateException {
    ...
    if (secondaryAccessions == null || secondaryAccessions.length == 0) return expandedAccessions;
      ...
      if (secAccn.getText().contains("-")) {
        ...
        if (startAccn == null || endAccn == null) {
          throw new IllegalStateException(
              "Secondary accession format is invalid:" + secAccn.getText());
        }
