...
public class PseudogeneValueCheck extends EntryValidationCheck {
    ...
    public ValidationResult check(Entry entry) {
        ...
        Set<String> psedudogeneValues = new HashSet<>(Arrays.asList("processed", "unprocessed", "unitary", "allelic", "unknown"));

        Collection<Feature> pseudoGeneFeatures =
                SequenceEntryUtils.getFeaturesContainingQualifier(Qualifier.PSEUDOGENE_QUALIFIER_NAME, entry);
        ...
        for(Feature feat : pseudoGeneFeatures ) {
            ...
            if(psedogeneQual != null && !psedudogeneValues.contains(psedogeneQual.getValue())) {
                reportError(feat.getOrigin(), PSEUDOGENE_INVALID_VALUE_CHECK, psedogeneQual.getValue(), psedudogeneValues);
            }
        }
