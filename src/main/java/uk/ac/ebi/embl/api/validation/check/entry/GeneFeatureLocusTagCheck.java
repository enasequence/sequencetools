...
/**
 * Checks that there is a one-to-one association between gene features and locus tag qualifier
 * values
 */
...
public class GeneFeatureLocusTagCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    /**
     * which locus tag qualifiers are mapped to which gene features - should be an exclusive
     * association
     */
    ...
    for (Feature geneFeature : geneFeatures) {
      ...
      List<Qualifier> locusQualifiers =
          geneFeature.getQualifiers(Qualifier.LOCUS_TAG_QUALIFIER_NAME);
      if (locusQualifiers.size() > 0) {

        String locus_tag_name =
            locusQualifiers.get(0).getValue(); // got other checks for multiple locus_tags
  ...
  private ValidationMessage<Origin> reportError(
      ValidationResult result, Origin origin, String messageId, Object... params) {
    ValidationMessage<Origin> message =
