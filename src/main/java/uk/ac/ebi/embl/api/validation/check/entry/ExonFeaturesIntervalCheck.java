  ...
  public ValidationResult check(Entry entry) {
    ...
    for (Feature exonFeature : exonFeatures) {
      if (!(exonFeature.getLocations() instanceof RemoteLocation)) {
        ...
        if (!isremoteLocation) filteredExonFeatures.add(exonFeature);
      }
    }
    ...
    for (Feature feature : sortedFeatures) {
      ...
      if (prevEndLocation != null) {
        ...
        if (beginLocation - prevEndLocation == 1) {
          ...
          // if both feature has gene and locus_tag gene has priority, compare both gene and add
          // error message if same.
          // if one feature has one of them and the other is not add error.
          // if gene is missing in one of them and locus is available in both then just compare
          // local and add error if same
          if ((currGene == null && prevGene == null)) {
            if (currLocusTag == null
                || (prevLocusTag == null || currLocusTag.equalsIgnoreCase(prevLocusTag))) {
