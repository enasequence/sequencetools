...
@Description(
    "gap feature created for \"{0}\" n characters between \"{1}\" and \"{2}\" "
...
public class SequenceToGapFeatureBasesFix extends SequenceToGapFeatureBasesCheck {
  ...
  /**
   * a count of the number of gap features that are of length 100 (the default value for unknown)
   */
  ...
  protected void processMissingGapFeature(Entry entry, NRegion nRegion) {
    ...
    if (getEmblEntryValidationPlanProperty().getOptions().minGapLength != 0) {
      ERROR_THRESHOLD =
          getEmblEntryValidationPlanProperty().getOptions().minGapLength
              - 1; // >=minimum_gap_length
    } else {
    ...
    if (nRegion.getLength() > ERROR_THRESHOLD) {
      ...
      Location location =
          locationFactory.createLocalRange((long) nRegion.getStart(), (long) nRegion.getEnd());
      ...
      if (getEmblEntryValidationPlanProperty().validationScope.get().isInGroup(Group.ASSEMBLY)
          || assemblyGapFeatures.size() > 0) {
        newGapFeature = features.createFeature(Feature.ASSEMBLY_GAP_FEATURE_NAME);
        newGapFeature.setLocations(compoundJoin);
        newGapFeature.setSingleQualifierValue(
            Qualifier.ESTIMATED_LENGTH_QUALIFIER_NAME, Integer.toString(nRegion.getLength()));
        newGapFeature.setSingleQualifierValue(Qualifier.GAP_TYPE_QUALIFIER_NAME, "unknown");
        ...
        newGapFeature = features.createFeature(Feature.GAP_FEATURE_NAME);
        newGapFeature.setLocations(compoundJoin);
        newGapFeature.setSingleQualifierValue(
            Qualifier.ESTIMATED_LENGTH_QUALIFIER_NAME, Integer.toString(nRegion.getLength()));
        entry.addFeature(newGapFeature);
    ...
    if (newGapFeatureCount != 0) {
      ...
      if (newGapFeatureCount == unknownEstimatedLength
        ...
        for (Feature gapFeature : gapFeatures) {
          Qualifier estimatedLengthQualifier =
              getQualifier(Qualifier.ESTIMATED_LENGTH_QUALIFIER_NAME, gapFeature);
          if (estimatedLengthQualifier != null
              && estimatedLengthQualifier.getValue().equals(GAP_ESTIMATED_LENGTH_STRING)) {
      ...
      if (unknownLength) {
        reportMessage(Severity.FIX, entry.getOrigin(), ESTIMATED_LENGTH_ID, newGapFeatureCount);
      }
      reportMessage(Severity.FIX, entry.getOrigin(), COUNT_MESSAGE_ID, newGapFeatureCount);
    }
  }
}
