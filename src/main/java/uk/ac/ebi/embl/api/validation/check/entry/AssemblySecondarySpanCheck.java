...
public class AssemblySecondarySpanCheck extends EntryValidationCheck {
  ...
  private static final String MESSAGE_ID = "AssembleSecondarySpanCheck-1";
  ...
  public ValidationResult check(Entry entry) {
    ...
    for (Assembly assembly : entry.getAssemblies()) {
      ...
      if (beginPosition < 0 || beginPosition > entry.getSequence().getLength()) {
        result.append(
            EntryValidations.createMessage(entry.getOrigin(), Severity.ERROR, MESSAGE_ID));
      }
      if (endPosition < 0 || endPosition > entry.getSequence().getLength()) {
        result.append(
            EntryValidations.createMessage(entry.getOrigin(), Severity.ERROR, MESSAGE_ID));
      }
    }
