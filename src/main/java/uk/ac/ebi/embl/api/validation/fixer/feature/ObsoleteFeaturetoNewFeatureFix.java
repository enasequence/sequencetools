...
public class ObsoleteFeaturetoNewFeatureFix extends FeatureValidationCheck {
  ...
  public ValidationResult check(Feature feature) {
    DataSet obsoleteFeatureDataSet =
        GlobalDataSets.getDataSet(GlobalDataSetFile.OBSOLETE_FEATURE_TO_FEATURE);
    ...
    for (DataRow dataRow : obsoleteFeatureDataSet.getRows()) {
      ...
      if (featureName.equals(obsoleteFeature)) {
        ...
        reportMessage(
            Severity.FIX,
            feature.getOrigin(),
            ObsoleteFeaturetoFeatureFix_ID_1,
            obsoleteFeature,
            newFeature,
            newFeatureQualifierName,
            newFeatureQualifierValue);
      }
    }
    return result;
  }
