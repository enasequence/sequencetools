...
public class DivisionFix extends EntryValidationCheck {
  ...
  private static final String DIVISION_FIX_ID = "DivisionFix_1";
  private static final String DIVISION_NOT_FOUND_ID = "DivisionFix_2";
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    if (shouldSetDivision(entry)) {
      try {
        ...
        String division =
            (primarySF.isTransgenic())
                ? "TGN"
                : (null
                        != primarySF.getSingleQualifier(
  ...
  private String getDivisionByScientificName(SourceFeature primarySF) {
    ...
    if (empty(division)) {
      ...
      if (!(taxonList =
              new TaxonomyClient().getTaxonsByScientificName(primarySF.getScientificName()))
          .isEmpty()) {
  ...
  private String getDivisionFromCache(String divisionKey) {
    ...
    return EmblEntryValidationPlan.divisionCache.get(divisionKey) != null
        ? EmblEntryValidationPlan.divisionCache.get(divisionKey)
        : null;
  }
