...
public class DataclassCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    for (DataRow row : GlobalDataSets.getDataSet(GlobalDataSetFile.DATACLASS).getRows()) {
      ...
      if (validDataclass.equals(entryDataclass)) sflag = true;
    }
    if (!sflag) {
    ...
    try {
      ...
      ArrayList<String> keywordDataclasses =
          DataclassProvider.getKeywordDataclass(
              entry, GlobalDataSets.getDataSet(GlobalDataSetFile.KEYWORD_DATACLASS));

      if (keywordDataclasses.size() == 1
          && !keywordDataclasses.get(0).equals("XXX")
          && !keywordDataclasses.get(0).equals(entryDataclass)) {
        reportError(
            entry.getOrigin(),
            KEYWORD_DATACLASS_MESSAGE_ID,
            keywordDataclasses.get(0),
            entryDataclass);
      }

      String accessionDataclass =
          DataclassProvider.getAccessionDataclass(entry.getPrimaryAccession());
      if (accessionDataclass != null && !accessionDataclass.equals(entryDataclass)) {
        reportError(
            entry.getOrigin(), ACCESSION_DATACLASS_MESSAGE_ID, accessionDataclass, entryDataclass);
      }
    } catch (Exception e) {
