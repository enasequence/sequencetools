...
public class NonContoAGPFix extends EntryValidationCheck {
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    try {
      ...
      if (entry.getSequence() != null
          ...
          && SequenceEntryUtils.getFeatures(Feature.ASSEMBLY_GAP_FEATURE_NAME, entry).size()
              != 0) // non-CON entries
      {
        ...
        if (assemblyGapFeatures.size() == 1) {
          ...
          agpRows.add(
              generateComponentAgpRow(prevGapEnd, gapBegin, componentOrder++, contig_type, entry));
          agpRows.add(generateGapAgpRow(assemblyGapFeature, componentOrder++, entry));
          agpRows.add(
              generateComponentAgpRow(gapEnd, seqLength + 1, componentOrder++, contig_type, entry));
        } else {
          for (int i = 0; i < assemblyGapFeatures.size(); i++) {
            ...
            if (i == 0) {
              if (gapBegin != 1) {
                agpRows.add(
                    generateComponentAgpRow(
                        prevGapEnd, gapBegin, componentOrder++, contig_type, entry));
              }
              agpRows.add(generateGapAgpRow(assemblyGapFeature, componentOrder++, entry));
              ...
              agpRows.add(
                  generateComponentAgpRow(
                      gapEnd, seqLength + 1, componentOrder++, contig_type, entry));
            } else {
              agpRows.add(
                  generateComponentAgpRow(
                      prevGapEnd, gapBegin, componentOrder++, contig_type, entry));
  ...
  AgpRow generateGapAgpRow(Feature gapFeature, int order, Entry scaffoldEntry)
      ...
      if (Qualifier.GAP_TYPE_QUALIFIER_NAME.equals(gapQualifier.getName())) {
        if (GlobalDataSets.gapType.containsKey(gapQualifier.getValue())) {
          ...
          throw new ValidationEngineException(
              String.format("Invalid gap type %s", gapQualifier.getValue()),
    ...
    agpRow.setObject_acc(
        scaffoldEntry.getPrimaryAccession() + "." + scaffoldEntry.getSequence().getVersion());
    agpRow.setObject(scaffoldEntry.getSubmitterAccession());
  ...
  private AgpRow generateComponentAgpRow(
      Long prevGapEnd, Long gapBegin, int order, String contig_type, Entry scaffoldEntry) {
    AgpRow agpRow = new AgpRow();
    agpRow.setObject(scaffoldEntry.getSubmitterAccession());
    agpRow.setObject_acc(
        scaffoldEntry.getPrimaryAccession() + "." + scaffoldEntry.getSequence().getVersion());
