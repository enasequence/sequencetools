...
public class NonContoAGPFix extends EntryValidationCheck
{
	public ValidationResult check(Entry entry) throws ValidationEngineException
	{
	 ...
	 AgpRow generateGapAgpRow(Feature gapFeature,int order,Entry scaffoldEntry) throws ValidationEngineException
	{
			 ...
			 for ( int i = 0; i < gapQualifiers.size(); ++i )
			 {
				 ...
				 if (Qualifier.GAP_TYPE_QUALIFIER_NAME.equals(gapQualifier.getName())) {
					 if (GlobalDataSets.gapType.containsKey(gapQualifier.getValue())) {
						 ...
						 throw new ValidationEngineException(String.format("Invalid gap type %s", gapQualifier.getValue()), ValidationEngineException.ReportErrorType.VALIDATION_ERROR);
					 }
				 }
