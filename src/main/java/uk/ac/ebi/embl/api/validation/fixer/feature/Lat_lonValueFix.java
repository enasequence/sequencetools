...
public class Lat_lonValueFix extends FeatureValidationCheck {
  ...
  public ValidationResult check(Feature feature) {
    ...
    String regex =
        "^\\s*(-{0,1}\\s*\\d+(\\.\\d+){0,1})\\s+(S|N)\\s*,{0,1}\\s+(-{0,1}\\s*\\d+(\\.\\d+){0,1})\\s+(W|E)\\s*$";
    Pattern pattern = Pattern.compile(regex);

    for (Qualifier qualifier : latlonQualifiers) {
      if (qualifier.getValue() == null) continue;
      ...
      if (matcher.find()) {
        ...
        if (latValue != null) {
          ...
          if (direcSN != null) newValue += direcSN + " ";
        }

        if (lonValue != null) {
          newValue += getLatLonValue(lonValue) + " ";
          if (direcWE != null) newValue += direcWE;
        }
        ...
        reportMessage(
            Severity.FIX, qualifier.getOrigin(), LatLonQualRemovedFix, qualifier.getValue());
      }
    }
    return result;
  }
