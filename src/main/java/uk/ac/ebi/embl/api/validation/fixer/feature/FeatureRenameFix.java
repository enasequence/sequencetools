...
public class FeatureRenameFix extends FeatureValidationCheck {
  ...
  private final HashMap<String, String> featureMap = new HashMap<String, String>();

  private static final String FEATURE_RENAME_FIX_ID = "FeatureRenameFix";

  public FeatureRenameFix() {}

  private void init() {
    DataSet featureRenameSet = GlobalDataSets.getDataSet(GlobalDataSetFile.FEATURE_RENAME);
    if (featureRenameSet != null) {

      for (DataRow frdataRow : featureRenameSet.getRows()) {
        String oldFeatureName = Utils.parseTSVString(frdataRow.getString(0));
        String newFeatureName = Utils.parseTSVString(frdataRow.getString(1));
  ...
  public ValidationResult check(Feature feature) {
    ...
    if (featureMap.containsKey(feature.getName())) {
      ...
      /**
       * special case - we only rename repeat_region if the /mobile_element qualifier also exists
       * (EMD-2607)
       */
  ...
  private void renameFeature(Feature feature, String newName) {
    ...
    reportMessage(Severity.FIX, feature.getOrigin(), FEATURE_RENAME_FIX_ID, oldName, newName);
  }
}
