 ...
 */
@Description(
    "Features sharing locus_tag \\\"{0}\\\" are associated with \\\"gene\\\" qualifiers with different values (\\\"{1}\\\" and \\\"{2}\\\")"
...
public class LocusTagAssociationCheck extends EntryValidationCheck {
  ...
  protected static final String MESSAGE_ID_DIFFERENT_GENE_SYNONYM_VALUES =
      "LocusTagAssociationCheck2";
  ...
  public ValidationResult check(Entry entry) {
    ...
    /**
     * If there is a CDS or a gene feature, prioritize the list of gene_synonyms from these (CDS
     * first, then Gene)
     */
    ...
    for (Feature locusFeature : locusFeatures) {
        ...
        if (locusTag2Gene.containsKey(locus_tag_name)) {
          ...
          if (!registeredGeneName.equals(currentGeneName)) { // is associated with more than 1 gene
            reportError(
                result,
                locusFeature,
                MESSAGE_ID_DIFFERENT_GENE_VALUES,
                locus_tag_name,
                registeredGeneName,
                currentGeneName);
          }
      ...
      if (locusTag2GeneSynonyms.containsKey(locus_tag_name)) {
        ...
        if (!geneSynonymValues.isEmpty()
            && !masterGenes.isEmpty()
            && !Arrays.deepEquals(masterGenes.toArray(), geneSynonymValues.toArray())) {
          ValidationMessage<Origin> message =
              reportError(
                  result, locusFeature, MESSAGE_ID_DIFFERENT_GENE_SYNONYM_VALUES, locus_tag_name);
          String masterValues =
              Utils.paramArrayToCuratorTipString(
                  masterGenes.toArray(new String[masterGenes.size()]));
          String theseValues =
              Utils.paramArrayToCuratorTipString(
                  geneSynonymValues.toArray(new String[geneSynonymValues.size()]));
  ...
  private void setMasterLocusFeature(
      HashMap<String, List<String>> locusTag2GeneSynonyms,
      HashMap<String, String> locusTag2Gene,
      Feature feature) {
    String locus_tag_name =
        feature.getQualifiers(Qualifier.LOCUS_TAG_QUALIFIER_NAME).get(0).getValue();
   ...
   */
  private ValidationMessage<Origin> reportError(
      ValidationResult result, Feature feature, String messageId, Object... params) {
    ValidationMessage<Origin> message =
