...
public class MoltypeExistsCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    if (entry.getSequence() == null && entry.getPrimarySourceFeature() == null) return result;
    ...
    if (entry.getPrimarySourceFeature() != null) {
      Qualifier molTypeQualifier =
          entry.getPrimarySourceFeature().getSingleQualifier(Qualifier.MOL_TYPE_QUALIFIER_NAME);
      if (molTypeQualifier != null) molType = molTypeQualifier.getValue();
    }

    if (molType == null) {
      reportError(
          entry.getOrigin(),
          MOLTYPE_MISSING_MESSAGE_ID,
