...
@Description(
    "sequence length must not be shorter than 100 bps"
...
public class SequenceLengthCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    if (dataclass != null
        && (entry.getDataClass().equals(Entry.PAT_DATACLASS)
    ...
    if (sequenceExistsCheck.check(entry).isValid()) {
      ...
      if (entry.getIdLineSequenceLength() > 0 && entry.getIdLineSequenceLength() != length) {
        reportError(
            entry.getOrigin(),
            SEQUENCE_LENGTH_MISMATCH_MESSAGE_ID,
            entry.getIdLineSequenceLength(),
            length);
      }

      if (length < 200
          && SequenceEntryUtils.isQualifierWithValueAvailable(
              Qualifier.NCRNA_CLASS_QUALIFIER_NAME, "lncRNA", entry))
      ...
      if (dataclass != null && dataclass.equals(Entry.GSS_DATACLASS)) {
        if (length < MIN_SEQUENCE_LENGTH_GSS || length > MAX_SEQUENCE_LENGTH_GSS) {
          reportError(
              entry.getOrigin(),
              SEQUENCE_LENGTH_GSS_MESSAGE_ID,
              MIN_SEQUENCE_LENGTH_GSS,
              MAX_SEQUENCE_LENGTH_GSS,
        ...
        if (length < MIN_SEQUENCE_LENGTH_TSA) {
          ...
          reportError(
              entry.getOrigin(),
              SEQUENCE_LENGTH_TSA_MESSAGE_ID,
              MIN_SEQUENCE_LENGTH_TSA,
              entry.getSubmitterAccession());
        }
        ...
        if (entry.getSequence().getLength() < minSeqLength) {
          if (SequenceEntryUtils.isFeatureAvailable(Feature.ncRNA_FEATURE_NAME, entry)
              && entry.getSequence().getLength() < minSeqLength) {

            return result;
          }
          List<Qualifier> satelliteQualifierList =
              SequenceEntryUtils.getQualifiers(Qualifier.SATELLITE_QUALIFIER_NAME, entry);
          ...
          reportError(
              result,
              entry,
              SEQUENCE_LENGTH_MESSAGE_ID,
              minSeqLength,
              entry.getSubmitterAccession() == null
  ...
  private ValidationMessage<Origin> reportError(
      ValidationResult result, Entry entry, String messageId, Object... params) {
    ValidationMessage<Origin> message =
        EntryValidations.createMessage(entry.getOrigin(), Severity.ERROR, messageId, params);
