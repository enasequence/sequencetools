...
public class MasterEntryService {
  ...
  public Entry createMasterEntry(SubmissionOptions options, ValidationResult validationResult)
    ...
    EmblEntryValidationPlan validationPlan =
        new EmblEntryValidationPlan(options.getEntryValidationPlanProperty());
    validationResult.append(validationPlan.execute(masterEntry));
    return masterEntry;
  }

  Entry getMasterEntryFromSubmittedXml(SubmissionOptions options, EraproDAOUtils eraproDAOUtils)
      throws ValidationEngineException {

    try {
      Entry masterEntry =
          eraproDAOUtils.createMasterEntry(options.analysisId.get(), getAnalysisType(options));
      if (masterEntry != null) {
  ...
  private Entry getMasterEntryFromWebinCli(SubmissionOptions options)
      throws ValidationEngineException {
    if (!options.assemblyInfoEntry.isPresent()) {
      throw new ValidationEngineException(
          "SubmissionOption assemblyInfoEntry must be given to generate master entry");
    }

    if (!options.source.isPresent()) {
      throw new ValidationEngineException(
          "SubmissionOption source must be given to generate master entry");
    }
    ...
    if (analysisType == AnalysisType.TRANSCRIPTOME_ASSEMBLY) {
      ...
      masterEntry
          .getSequence()
          .setMoleculeType(
              infoEntry.getMoleculeType() == null ? "genomic DNA" : infoEntry.getMoleculeType());
    }
    ...
    if (options.context.get() == Context.genome)
      masterEntry.setDescription(
          new Text(
              SequenceEntryUtils.generateMasterEntryDescription(
                  source, AnalysisType.SEQUENCE_ASSEMBLY, infoEntry.isTpa())));
