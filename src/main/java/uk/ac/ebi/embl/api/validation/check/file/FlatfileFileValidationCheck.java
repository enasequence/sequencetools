...
public class FlatfileFileValidationCheck extends FileValidationCheck {
    ...
    try (BufferedReader fileReader = CommonUtil.bufferedReaderFromFile(submissionFile.getFile());
      ...
      while (entryReader.isEntry()) {
        ...
        if (StringUtils.isBlank(entry.getSubmitterAccession())
            && getOptions().context.get() == Context.genome) {
      ...
      getReporter()
          .writeToFile(getReportFile(submissionFile), Severity.ERROR, ex.getMessage(), origin);
      throw new ValidationEngineException(ex.getMessage(), ex);
    }

    if (validationResult.isValid()) registerFlatfileInfo();
    return validationResult;
  }
  ...
  private void registerFlatfileInfo() throws ValidationEngineException {
    AssemblySequenceInfo.writeMapObject(
        sharedInfo.flatfileInfo, options.processDir.get(), AssemblySequenceInfo.flatfilefileName);
  }
}
