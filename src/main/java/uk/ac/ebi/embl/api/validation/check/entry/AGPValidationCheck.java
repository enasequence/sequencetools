...
public class AGPValidationCheck extends EntryValidationCheck {
  ...
  private static final String MESSAGE_KEY_SAME_COMPONENT_AND_OBJECT_ERROR = "AGPValidationCheck-15";
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    for (AgpRow agpRow : entry.getSequence().getAgpRows()) {
      ...
      if (part_number != (prevPartNumber + 1)) {
        reportError(
            agpRow.getOrigin(), MESSAGE_KEY_PART_NUMBER_ERROR, entry.getSubmitterAccession());
      }
      ...
      if (component_type_id == null) {
        ...
        if (!ArrayUtils.contains(componentTypeArray, component_type_id)) {
          reportError(
              agpRow.getOrigin(), MESSAGE_KEY_COMPONENT_TYPE_ID_ERROR, agpRow.getComponent_id());
        }
      }
      ...
      if (component_type_id != null && agpRow.isGap()) {
        ...
        if (gap_type == null) {
          reportError(
              agpRow.getOrigin(), MESSAGE_KEY_GAP_TYPE_ERROR, entry.getSubmitterAccession());
        } else {
        ...
        if (orientation == null) {
          ...
          if (!ArrayUtils.contains(orientationArray, orientation)) {
            reportError(
                agpRow.getOrigin(), MESSAGE_KEY_ORIENTATION_ERROR, agpRow.getComponent_id());
          }
        }
        ...
        if (agpRow.getComponent_id().equalsIgnoreCase(agpRow.getObject())) {
          reportError(
              agpRow.getOrigin(),
              MESSAGE_KEY_SAME_COMPONENT_AND_OBJECT_ERROR,
              agpRow.getComponent_id());
        } else {
          ...
          if (getEmblEntryValidationPlanProperty().isRemote.get()) {
            if (getEmblEntryValidationPlanProperty().assemblySequenceInfo.get().size() == 0) {
              throw new ValidationEngineException(
                  "AssemblySequenceInfo must be given to validate AGP file");
            }
          }
          if (getEmblEntryValidationPlanProperty().assemblySequenceInfo.get().size() > 0) {
            if (getEmblEntryValidationPlanProperty()
                    ...
                    .get()
                    .get(agpRow.getComponent_id().toUpperCase())
                == null)
              reportError(
                  agpRow.getOrigin(), MESSAGE_KEY_COMPONENT_VALID_ERROR, agpRow.getComponent_id());
            else {
              sequenceLength =
