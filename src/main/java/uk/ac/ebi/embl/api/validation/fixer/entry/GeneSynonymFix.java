 ...
 */
@Description(
    "Added gene_synonym \\\"{0}\\\" to feature sharing locus_tag/gene \\\"{1}\\\" - to create a stable list of gene_synonyms"
...
public class GeneSynonymFix extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    locusAndGeneFeatures.addAll(
        SequenceEntryUtils.getFeaturesContainingQualifier(GENE_QUALIFIER_NAME, entry));
    ...
    /**
     * keep a list of locus tags that we have decided should not be considered due to unstable
     * associations in CDS features - don't even try with these ones
     */
    ...
    for (Feature currentFeature : locusAndGeneFeatures) {
      if (currentFeature.getName().equals(Feature.CDS_FEATURE_NAME)) {
        ...
        if (!currentFeature.getQualifiers(LOCUS_TAG_QUALIFIER_NAME).isEmpty()) {
          identifier =
              currentFeature.getQualifiers(Qualifier.LOCUS_TAG_QUALIFIER_NAME).get(0).getValue();
          ...
          continue; // this should not be possible as the list was built with features containing at
          // least 1 of the above
        }

        /**
         * look at all occurrences of any given LOCUS_TAG or GENE_QUALIFIER name - if there are
         * mismatches between the gene_synonym qualifiers, exclude from the fix - they are
         * unreliable and we cant decide which of the lists of syninyms we should use to overwrite
         * the other features.
         */
        ...
        if (identifier2GeneSynonyms.containsKey(identifier)) {
          ...
          for (String registeredSynonym : masterSynonyms) {
            if (!SequenceEntryUtils.isQualifierWithValueAvailable(
                GENE_SYNONYM_NAME, registeredSynonym, currentFeature)) {
              currentFeature.addQualifier(
                  qualifierFactory.createQualifier(GENE_SYNONYM_NAME, registeredSynonym));
              reportMessage(
                  Severity.FIX,
                  currentFeature.getOrigin(),
                  MESSAGE_ID_ADDED_SYNONYM,
                  registeredSynonym,
                  identifier);
            }
          }
          ...
          /**
           * then look at all the synonyms associated with this feature and remove those not present
           * in the master list
           */
  ...
  private void setMasterSynonymFeature(
      HashMap<String, List<String>> locusTag2GeneSynonyms, String identifier, Feature feature) {
