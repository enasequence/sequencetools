...
@Description(
    "Qualifier \\\"{0}\\\" must have one of values {1} when qualifier \\\"{2}\\\" value starts with \\\"{3}\\\".")
@ExcludeScope(validationScope = {ValidationScope.NCBI, ValidationScope.NCBI_MASTER})
public class QualifierValueRequiredQualifierStartsWithValueCheck extends FeatureValidationCheck {

  private static final String MESSAGE_ID = "QualifierValueRequiredQualifierStartsWithValueCheck";

  public QualifierValueRequiredQualifierStartsWithValueCheck() {}

  public ValidationResult check(Feature feature) {
    DataSet dataSet =
        GlobalDataSets.getDataSet(GlobalDataSetFile.QUALIFIER_VALUE_REQ_QUALIFIER_STARTSWITH_VALUE);
    ...
    for (DataRow row : dataSet.getRows()) {
      ...
      for (Qualifier qualifier : feature.getQualifiers()) {
        if (qualifier.getValue() != null
            && qualifier.getName().equals(qualifierName)
            && qualifier.getValue().startsWith(qualifierValue)) {
