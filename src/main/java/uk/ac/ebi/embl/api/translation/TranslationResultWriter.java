...
public class TranslationResultWriter {
    ...
    private void writeBlock(Writer writer, 
        ...
        for (int i = begin ; i < end ; ++i) {
            ...
            if (!codon.getCodon().equals("nnn") &&
                unAmbiguousCodons.length > maxUnambiguousCodons) {
        ...
        if (maxUnambiguousCodons > 1) {            
            ...
            for (int j = 0 ; j < maxUnambiguousCodons ; ++j ) {
                for (int i = begin ; i < end ; ++i) {
                    UnAmbiguousCodon[] unAmbiguousCodons =
                        result.getCodons().get(i).getUnAmbiguousCodons();
                ...
                for (int i = begin ; i < end ; ++i) {
                    ...
                    if (!result.getCodons().get(i).getCodon().equals("nnn") &&
                        ...
                        unAmbiguousCodons.length > 1 &&
                        unAmbiguousCodons.length > j) {
                        writeUnAmbiguousAminoAcid(writer, 
                            unAmbiguousCodons[j]);
                    }
                    else {
                        writer.append("     ");
    ...
    private void writeAmbiguousCodonStart(Writer writer, 
        int begin, int end) throws IOException {
        for (int i = begin ; i < end ; ++i) {
            Codon codon = result.getCodons().get(i);
            UnAmbiguousCodon[] unAmbiguousCodons = codon.getUnAmbiguousCodons();
    ...
    private void writeAmbiguousCodonEnd(Writer writer, 
        int begin, int end) throws IOException {
        for (int i = begin ; i < end ; ++i) {
            Codon codon = result.getCodons().get(i);
            UnAmbiguousCodon[] unAmbiguousCodons = codon.getUnAmbiguousCodons();
    ...
    private void writeUnAmbiguousAminoAcid(Writer writer, 
        UnAmbiguousCodon unAmbiguousCodon)  throws IOException {
        writer.append(" ");
    ...
    private void writeTranslatedAminoAcid(Writer writer, int i)  throws IOException {
        ...
        boolean displayCodonException = (codon.getUnAmbiguousCodons()
            .length == 1 && codon.isCodonException());
