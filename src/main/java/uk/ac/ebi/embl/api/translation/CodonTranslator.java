...
/**
 * Translates a codon to an amino acid. The bases are encoded using lower case single letter JCBN
 * abbreviations and the amino acids are encoded using upper case single letter JCBN abbreviations.
 */
public class CodonTranslator {
  ...
  private void translateCodon(Codon codon, Map<String, Character> codonMap)
  ...
  public boolean isDegenerateStopCodon(Codon codon) throws ValidationException {
    return isDegenerateCodon(codon, translationTable.getOtherCodonMap(), '*');
  }

  public boolean isDegenerateStartCodon(Codon codon) throws ValidationException {
    return isDegenerateCodon(codon, translationTable.getStartCodonMap(), 'M');
  }

  private boolean isDegenerateCodon(
      Codon codon, Map<String, Character> codonMap, Character aminoAcid)
      throws ValidationException {
    List<UnAmbiguousCodon> unAmbiguousCodons = getUnAmbiguousCodons(codon);
    for (UnAmbiguousCodon unAmbiguousCodon : unAmbiguousCodons) {
      Character newAminoAcid = null;
  ...
  private List<UnAmbiguousCodon> getUnAmbiguousCodons(Codon codon) {
    ...
    List<Character> unAmbiguousBases1 = getUnAmbiguousBases(codon.getCodon().charAt(0));
    // for (Character t : unAmbiguousBases1) {
    // System.out.println("unAmbiguousBases1:" + t);
    // }
    List<Character> unAmbiguousBases2 = getUnAmbiguousBases(codon.getCodon().charAt(1));
    // for (Character t : unAmbiguousBases2) {
    // System.out.println("unAmbiguousBases2:" + t);
    // }
    List<Character> unAmbiguousBases3 = getUnAmbiguousBases(codon.getCodon().charAt(2));
