...
/**
 * Validator for percent_query_cover column values. Rule: Must be a float or a range of floats
 * (e.g., "100" or "98.7-99.2"). The percentage coverage of the query sequence that aligns with a
 * reference sequence. Optional field - can be empty.
 */
public class PercentQueryCoverValidator implements ColumnValidator {

  private static final String COLUMN_NAME = "percent_query_cover";
  ...
  public ValidationResult validate(String value, int rowNumber, int columnIndex) {
    ...
    // Check if value is a valid float or range of floats (e.g., "100" or "98.7-99.2")
    ...
    if (!value.matches("^\\d+(\\.\\d+)?(\\s*-\\s*\\d+(\\.\\d+)?)?$")) {
      ...
      message.setMessage(
          String.format(
              "Row %d, column %d (%s): Invalid value '%s' - must be a number or range (e.g., '100' or '98.7-99.2')",
