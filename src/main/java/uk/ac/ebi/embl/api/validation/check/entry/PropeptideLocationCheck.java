...
public class PropeptideLocationCheck extends EntryValidationCheck
{
	...
	private final static String LENGTH_MULTIPLE_OF_THREE_MESSAGE_ID = "PropeptideLocationCheck1";
	private final static String WITHIN_CDS_MESSAGE_ID = "PropeptideLocationCheck2";
	private final static String WITHIN_PEPTIDE_MESSAGE_ID = "PropeptideLocationCheck3";
	...
	public ValidationResult check(Entry entry)
	{
		...
		List<Feature> propetideFeatures=SequenceEntryUtils.getFeatures(Feature.PROPETIDE_FEATURE_NAME, entry);

		 if(propetideFeatures==null||propetideFeatures.size()==0)
			 return result;
		...
		for (Feature propetideFeature : propetideFeatures)
		{
			...
			{
				if(propetideFeature.getLocations().getMinPosition()>=feature.getLocations().getMinPosition()
				   &&propetideFeature.getLocations().getMaxPosition()<=feature.getLocations().getMaxPosition())
				{
			...
			{
				if((propetideFeature.getLocations().getMinPosition()>feature.getLocations().getMinPosition()&&propetideFeature.getLocations().getMinPosition()<feature.getLocations().getMaxPosition())
				   ||(propetideFeature.getLocations().getMaxPosition()>feature.getLocations().getMinPosition()&&propetideFeature.getLocations().getMaxPosition()<feature.getLocations().getMaxPosition()))
				{
