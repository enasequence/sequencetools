...
public class FeatureQualifierRenameFix extends FeatureValidationCheck {
  ...
  private void init() {
    DataSet featureQualifierRenameSet =
        GlobalDataSets.getDataSet(GlobalDataSetFile.FEATURE_QUALIFIER_RENAME);

    if (featureQualifierRenameSet != null) {

      for (DataRow frdataRow : featureQualifierRenameSet.getRows()) {
        String oldFeatureQualifierName = Utils.parseTSVString(frdataRow.getString(0));
        String newFeatureQualifierName = Utils.parseTSVString(frdataRow.getString(1));
        //				System.out.println("oldFeatureName=" + oldFeatureQualifierName
        //						+ "newFeatureName" + newFeatureQualifierName);
        featureQualifierMap.put(oldFeatureQualifierName, newFeatureQualifierName);
      }
    }
  }
  ...
  public ValidationResult check(Feature feature) {
    ...
    for (Qualifier qualifier : feature.getQualifiers()) {
      ...
      if (featureQualifierMap.containsKey(qualifierName)) {
        ...
        String newqualifiervalue = featureQualifierMap.get(qualifierName);
  ...
  private void renameFeatureQualifier(Qualifier qualifier, String newName) {
    ...
    if (oldName.equals(Qualifier.LABEL_QUALIFIER_NAME)) {
      ...
      //            System.out.println("\'"+qualifier.getName()+"\' value has been changed to:
      // \'"+qualifier.getValue()+"\'");
    }

    reportMessage(
        Severity.FIX, qualifier.getOrigin(), FEATURE_QUALIFIER_RENAME_FIX_ID, oldName, newName);
  }
}
