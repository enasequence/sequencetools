...
public class CdsTranslator {
   ...
   * @return
   */
  public ExtendedResult<TranslationResult> translate(CdsFeature cds, Entry entry)
      throws RepositoryException {

    ExtendedResult<TranslationResult> validationResult = new ExtendedResult<TranslationResult>();
    if (cds == null || cds.getLocations() == null) return validationResult;
    ...
    if (entry.getSequence() == null || entry.getSequence().getSequenceByte() == null) {
      validationResult.append(
          EntryValidations.createMessage(cds.getOrigin(), Severity.ERROR, "CDSTranslator-5"));
    }
    ...
    if (cds.isException() && cds.getTranslation() == null) {
      ...
      validationResult.append(
          EntryValidations.createMessage(cds.getOrigin(), Severity.ERROR, "CDSTranslator-1"));
    }
    if (validationResult.count(Severity.ERROR) > 0) {
      return validationResult;
    }
    ...
    if (!checkLocations(entry.getSequence().getLength(), cds.getLocations())) {
      validationResult.append(
          EntryValidations.createMessage(cds.getOrigin(), Severity.ERROR, "CDSTranslator-4"));
    }
    ...
    try {
      ...
      ExtendedResult<TranslationResult> extendedTranslatorResult =
          translator.translate(segment.getSequenceByte(), entry.getOrigin());
      ...
      if (expectedTranslation == null || expectedTranslation.length() == 0) {
        ...
        if (comparisonRes.left) {
          if (cds.isException()) {
            extendedTranslatorResult.append(
                EntryValidations.createMessage(
                    cds.getOrigin(), Severity.WARNING, "CDSTranslator-3"));
          }
          ...
          if (acceptTranslation || comparisonRes.right > 0) {
            ...
            extendedTranslatorResult.append(
                EntryValidations.createMessage(
                    cds.getOrigin(), Severity.WARNING, "CDSTranslator-2"));
          } else {
            // Reject teh translation
            extendedTranslatorResult.append(
                EntryValidations.createMessage(
                    cds.getOrigin(), Severity.ERROR, "CDSTranslator-16"));
          }
        }
      }
      ...
      if (fixes != null && !fixes.isEmpty()) {
        fixes.forEach(
            f ->
                extendedTranslatorResult.append(
                    EntryValidations.createMessage(cds.getOrigin(), Severity.FIX, f)));
      }

      if (severity != null) {
        Collection<ValidationMessage<Origin>> messages = extendedTranslatorResult.getMessages();
  ...
  private void createTranslator(
    ...
    try {
      ...
      reportException(validationResult, e, entry, cds);
    }
  }
  ...
  private boolean checkLocations(long length, CompoundLocation<Location> locations) {
    for (Location location : locations.getLocations()) {
      ...
      if (location.getIntBeginPosition() == null || location.getIntBeginPosition() < 0) {
        return false;
      }
      if (location.getIntEndPosition() == null || location.getIntEndPosition() > length) {
        return false;
      }
    }
    return true;
  ...
  private void reportException(
      ValidationResult result, ValidationException exception, Entry entry, CdsFeature cds) {
  ...
  public ValidationResult configureFromFeature(
      CdsFeature feature, TaxonomyClient taxonomyClient, Entry entry) throws ValidationException {
    ValidationResult validationResult = new ValidationResult();
    Integer featureTranslationTable = null;
    if (feature != null) featureTranslationTable = feature.getTranslationTable();
    ...
    if (sourceFeature != null) {
      ...
      if (taxon != null) {
        ...
        if (StringUtils.equals(organelle, "mitochondrion")
          ...
          infoMessage =
              EntryValidations.createMessage(
                  entry.getOrigin(), Severity.INFO, "CDSTranslator-11", translationTable);
        }
      } else {
        if (featureTranslationTable != null) {
          ...
          infoMessage =
              EntryValidations.createMessage(
                  entry.getOrigin(), Severity.INFO, "CDSTranslator-13", translationTable);
        } else {
          String organelle = sourceFeature.getSingleQualifierValue("organelle");
          if (StringUtils.equals(organelle, "mitochondrion")) {
            translationTable = 2;
            ...
            infoMessage =
                EntryValidations.createMessage(
                    entry.getOrigin(), Severity.INFO, "CDSTranslator-15", translationTable);
          }
        }
      }
    }
    ...
    if (translationTable == null) {
      ...
      validationResult.append(
        ...
        // non-cds feature
        validationResult.append(
            EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-8"));
    ...
    if (!translationTable.equals(1) && !translator.isPeptideFeature()) {
      ...
      if (feature.getSingleQualifier(Qualifier.TRANSL_TABLE_QUALIFIER_NAME) == null) {
        Qualifier ttQual =
            new QualifierFactory().createTranslTableQualifier(String.valueOf(translationTable));
        feature.addQualifier(ttQual);
        validationResult.append(
            EntryValidations.createMessage(
                entry.getOrigin(), Severity.FIX, "CDSTranslator-17", translationTable));
      }
    }
    ...
    for (TranslExceptQualifier qualifier : translExceptQualifiers) {
      ...
      Integer relativeBeginPos = feature.getLocations().getRelativeIntPosition(beginPosition);
      Integer relativeEndPos = feature.getLocations().getRelativeIntPosition(endPosition);
      ...
      if (relativeBeginPos == null) {
        validationResult.append(
            EntryValidations.createMessage(
                feature.getOrigin(), Severity.ERROR, "CDSTranslator-6", beginPosition.toString()));
      } else if (relativeEndPos == null) {
        validationResult.append(
            EntryValidations.createMessage(
                feature.getOrigin(), Severity.ERROR, "CDSTranslator-7", endPosition.toString()));
      } else {
        translator.addTranslationException(
            relativeBeginPos, relativeEndPos, qualifier.getAminoAcid().getLetter());
      }
    }
    ...
    for (CodonQualifier qualifier : codonQualifiers) {
      translator.addCodonException(qualifier.getCodon(), qualifier.getAminoAcid().getLetter());
    }
