...
public class MasterEntryExistsCheck extends EntryValidationCheck {
  ...
  private static final String MASTER_EXISTS_MESSAGE_ID = "MasterEntryExistsCheck_1";
  private static final String MASTER_BIOSAMPLE_EXISTS_MESSAGE_ID = "MasterEntryExistsCheck_2";

  public ValidationResult check(Entry entry) throws ValidationEngineException {

    result = new ValidationResult();
    boolean isMasterExists = false;
    ...
    try {
      if (entryDAOUtils != null) {
        isMasterExists =
            entryDAOUtils.isEntryExists(getEmblEntryValidationPlanProperty().analysis_id.get());
      }
    ...
    if (entry.getBiosampleId() == null) {
      reportError(
          entry.getOrigin(),
          MASTER_BIOSAMPLE_EXISTS_MESSAGE_ID,
          getEmblEntryValidationPlanProperty().analysis_id.get());
    }
