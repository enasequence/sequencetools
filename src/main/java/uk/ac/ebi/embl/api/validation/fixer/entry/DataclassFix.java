...
@Description(
    "dataclass has been fixed to \"{0}\"" + "Dataclass Keyword \"{0}\" has been added to the entry")
...
public class DataclassFix extends EntryValidationCheck {
  ...
  public enum DataclassKeywords {
    ...
    public static List<Text> getKeywords(String dataclass) {
  ...
  public ValidationResult check(Entry entry) throws ValidationEngineException {
    ...
    for (DataRow row : dataSet.getRows()) {
      if (keywords.stream().anyMatch(kw -> kw.getText().equalsIgnoreCase(row.getString(1)))
        ...
        if (dataClassFromKw == null) dataClassFromKw = row.getString(0);
    ...
    if (dataclass == null || "XXX".equals(dataclass)) {
      try {
        ...
        if (accessionDataclass != null) {
          ...
          ArrayList<String> keywordDataclassList =
              DataclassProvider.getKeywordDataclass(entry, dataSet);
      ...
      for (Text dataclassKeyword : dataclassKeywords) {
        if (dataclassKeyword != null && !keywords.contains(dataclassKeyword)) {
          ...
          reportMessage(
              Severity.FIX, entry.getOrigin(), KEYWORD_FIX_ID, dataclassKeyword.getText());
        }
      }
    }
