...
@Description(
    "The feature has no location.\\Feature location missing.\\The begin and end position of a sequence span are in the wrong order."
...
public class FeatureLocationCheck extends FeatureValidationCheck {
  ...
  public ValidationResult check(Feature feature) throws ValidationEngineException {
    ...
    try {
      ...
      if (feature instanceof CdsFeature && ((CdsFeature) feature).isPseudo()) return result;
