...
@Description(
    "Qualifier \\\"{0}\\\" must have one of values {1} when qualifier \\\"{2}\\\" has value \\\"{3}\\\"in any feature.")
...
public class QualifierValueRequiredQualifierValueEntryCheck extends EntryValidationCheck {
  ...
  public ValidationResult check(Entry entry) {
    ...
    DataSet dataSet =
        GlobalDataSets.getDataSet(GlobalDataSetFile.QUALIFIER_VAL_REQUIRED_QUALIFIER_ENTRY);
    ...
    for (DataRow row : dataSet.getRows()) {
      ...
      if (qualifierName == null
          || requiredQualifierName == null
          || ArrayUtils.isEmpty(requiredQualifierValues)) {
        continue;
      }

      if (!SequenceEntryUtils.isQualifierWithValueAvailable(qualifierName, qualifierValue, entry))
        continue;
      ...
      for (Feature feature : entry.getFeatures()) {
        requiredQualifiers.addAll(feature.getQualifiers(requiredQualifierName));
      }
