  ...
  public ValidationResult check(SubmissionFile submissionFile) throws ValidationEngineException {
    ...
    try (BufferedReader fileReader = CommonUtil.bufferedReaderFromFile(submissionFile.getFile());
      ...
      if (!validateFileFormat(
          ...
          uk.ac.ebi.embl.api.validation.submission.SubmissionFile.FileType.FASTA)) {
      ...
      while (reader.isEntry()) {
        ...
        getOptions()
            .getEntryValidationPlanProperty()
            .fileType
            .set(uk.ac.ebi.embl.api.validation.FileType.FASTA);
        if (sharedInfo.hasAnnotationOnlyFlatfile) {
    ...
    if (validationResult.isValid()) registerFastaInfo();
    return validationResult;
  }

  private void registerFastaInfo() throws ValidationEngineException {
    AssemblySequenceInfo.writeMapObject(
        sharedInfo.fastaInfo, options.processDir.get(), AssemblySequenceInfo.fastafileName);
  }
