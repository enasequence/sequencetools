...
public class GFF3Mapper {
	...
	public List<Entry> mapGFF3ToEntry(GFF3RecordSet records) throws IOException {
		...
		ResourceBundle featureQualifiers = ResourceBundle
		...
		for (GFF3Record record : records.getRecords()) {
			...
			while (attributeIterator.hasNext()) {
				...
				if (featureQualifiers.containsKey(attributeKey)) {
					qualifier = qualifierFactory.createQualifier(
							featureQualifiers.getString(attributeKey),
							attributeValue);
			...
			Location location = locationFactory.createLocalRange((long) start,
					(long) end);
			...
			if (featureQualifiers.containsKey(featureType)) {
				feature = featureFactory.createFeature(featureQualifiers
						.getString(featureType).toString());
