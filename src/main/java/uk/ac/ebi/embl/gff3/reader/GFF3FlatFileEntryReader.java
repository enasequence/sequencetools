...
public class GFF3FlatFileEntryReader implements FlatFileReader<GFF3RecordSet> {
  ...
  public void readLines() throws IOException {
    ...
    /**
     * only enter loop if the first line read is a line - as there are no terminator characters we
     * have to decide when the file is finished this way, rather than looking for a terminator tag.
     */
    if (lineReader.isCurrentLine()) {
      while (true) {
        if (!lineReader.isCurrentLine()) {
          isEntry =
              true; // say its an entry when the file runs out - may add checks for format errors if
          // needed
          break;
        }
