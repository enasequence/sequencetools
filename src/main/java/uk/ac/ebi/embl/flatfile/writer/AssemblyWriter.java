...
public class AssemblyWriter extends FlatFileWriter {
  ...
  public boolean write(Writer writer) throws IOException {
    ...
    for (Assembly assembly : assemblies) {
      ...
      String locanSpanString =
          secondarySpan.getBeginPosition() + "-" + secondarySpan.getEndPosition();

      RemoteRange primarySpan = assembly.getPrimarySpan();
      String primaryIdentifierString =
          primarySpan.getVersion() == null
              ? primarySpan.getAccession()
              : primarySpan.getAccession() + "." + primarySpan.getVersion();

      String primarySpanString =
          primarySpan.getBeginPosition() + "-" + primarySpan.getEndPosition();
      ...
      String line =
          formatter
              .format(
                  "%-"
                      + LOCAL_SPAN_COLUMN_WIDTH
                      + "s%-"
                      + PRIMARY_IDENTIFIER_COLUMN_WIDTH
                      + "s%-"
