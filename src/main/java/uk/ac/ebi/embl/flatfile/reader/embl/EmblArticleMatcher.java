...
public class EmblArticleMatcher extends FlatFileMatcher {
	...
	public Article getArticle(Publication publication) {
		...
		else
		{
			...
			{
				article.setIssue(journal_volume_issue.substring(journal_volume_issue.lastIndexOf('(')+1, journal_volume_issue.length() - 1));
				String journal_volume=StringUtils.replace(journal_volume_issue,article.getIssue() , "");
				article.setVolume(journal_volume.substring(journal_volume.lastIndexOf(' ')+1, journal_volume.lastIndexOf('(')));
				article.setJournal(journal_volume.substring(0, journal_volume.lastIndexOf(' ')));
			} else if (journal_volume_issue.matches(".*\\d$"))
			{
				article.setVolume(journal_volume_issue.substring(journal_volume_issue.lastIndexOf(' ')+1, journal_volume_issue.length()));
				article.setJournal(journal_volume_issue.substring(0, journal_volume_issue.lastIndexOf(' ')));
			} else
			{
