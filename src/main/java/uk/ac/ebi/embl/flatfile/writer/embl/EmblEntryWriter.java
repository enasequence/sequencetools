...
public class EmblEntryWriter extends EntryWriter {
  ...
  public boolean write(Writer writer) throws IOException {
      ...
      if (feature instanceof SourceFeature) {
        ...
        if (taxon != null && taxon.getScientificName() != null) {
          if (!scientificName.contains(taxon.getScientificName())) {
            if (new EmblOrganismWriter(entry, (SourceFeature) feature, wrapType).write(writer)) {
              writer.write(SEPARATOR_LINE);
            }
          }
    ...
    if (null == entry.getSequence()
        || null == entry.getSequence().getSequenceByte()
        || entry.isNonExpandedCON()) writeFeatures(writer);

    if (new COWriter(entry, wrapType).write(writer)) {
      if (entry.getSequence() != null
          && entry.getSequence().getSequenceByte() != null
          && !entry.isNonExpandedCON()) {
        writer.write(SEPARATOR_LINE);
      }
    }
