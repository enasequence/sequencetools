...
public class RLWriter extends FlatFileWriter {
  ...
  public boolean write(Writer writer) throws IOException {
    ...
    if (publication instanceof Book) {
      writeBlock |=
          new BookWriter(entry, (Book) publication, wrapType, EmblPadding.RL_PADDING).write(writer);
    } else if (publication instanceof ElectronicReference) {
      writeBlock |=
          new ElectronicReferenceWriter(
                  entry, (ElectronicReference) publication, wrapType, EmblPadding.RL_PADDING)
              .write(writer);
    } else if (publication instanceof Article) {
      writeBlock |= new EmblArticleWriter(entry, (Article) publication, wrapType).write(writer);
    } else if (publication instanceof Patent) {
      writeBlock |= new EmblPatentWriter(entry, (Patent) publication, wrapType).write(writer);
    } else if (publication instanceof Submission) {
      writeBlock |=
          new EmblSubmissionWriter(entry, (Submission) publication, wrapType).write(writer);
    } else if (publication instanceof Thesis) {
      writeBlock |= new EmblThesisWriter(entry, (Thesis) publication, wrapType).write(writer);
    } else if (publication instanceof Unpublished) {
      writeBlock |= new EmblUnpublishedWriter(entry, wrapType).write(writer);
    }
    return writeBlock;
  }
}
