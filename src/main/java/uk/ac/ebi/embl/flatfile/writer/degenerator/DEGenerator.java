    ...
    public static ValidationResult writeDE(Entry entry) {
        ...
        if (isQualifierAvailable(ORGANELLE_QUALIFIER_NAME, primarySource)) {
            ...
            if (organelleQual.indexOf(":") != -1) {
                ...
                organelleQual = organelleQual.substring((organelleQual.indexOf(":") + 1), organelleQual.length());
            }
        ...
        if(dataClass == null){
            result.append(ValidationMessage.message(Severity.ERROR, NO_DATACLASS_MESSAGE).append(entry.getOrigin()));
            return result;
        }
...
//        System.out.println("dataClass = " + dataClass);
        ...
        if (dataClass.equals(Entry.EST_DATACLASS) ||
            ...
            body = dataClass;
            String uniquifierString = DEData.getUniquifierString(
                    Arrays.asList(
                            ISOLATE_QUALIFIER_NAME,
                            CLONE_QUALIFIER_NAME,
                            MAP_QUALIFIER_NAME),
                    primarySource);
            ...
            if (feature != null) {
                ...
                body = body.concat(createmRNAProductStringFromFeature(
                        feature,
                        result,
                        entry));

            } else {
                result.append(ValidationMessage.message(WARNING, NO_CDS_GENE_MESSAGE).append(entry.getOrigin()));
            }
            ...
            String uniquifierString = DEData.getUniquifierString(
                    Arrays.asList(ISOLATE_QUALIFIER_NAME, CLONE_QUALIFIER_NAME),
                    primarySource);
            body = body.concat(uniquifierString);
            ...
            String uniquifierString = DEData.getUniquifierString(
                    Arrays.asList(ISOLATE_QUALIFIER_NAME),
                    primarySource);
            body = body.concat(uniquifierString);
    ...
    private static String createmRNAProductStringFromFeature(Feature feature,
        ...
        if (productString != null && geneString != null) {
            ...
            result.append(ValidationMessage.message(WARNING, NO_GENE_OR_PRODUCT_MESSAGE).append(entry.getOrigin()));
        }

        return resultString;
    }

    private static String createSTDProductStringFromFeature(Feature feature,
                                                            ValidationResult result,
                                                            Entry entry) {
        String resultString = "";
        String geneString = DEData.getGeneString(feature);
