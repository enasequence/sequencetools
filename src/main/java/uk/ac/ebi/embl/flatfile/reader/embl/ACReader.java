...
/** Reader for the flat file AC lines. Accession number ranges will not be expanded. */
    ...
    for (String accession : FlatFileUtils.split(block, ";")) {
      if (isFirstAccession) {
        if (noPrimaryAccession) {
          ...
          if (idLinePrimaryAccession != null && !idLinePrimaryAccession.equalsIgnoreCase(accession))
            error("AC.1");
          else {
