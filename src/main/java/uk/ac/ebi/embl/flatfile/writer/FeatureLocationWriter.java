...
public class FeatureLocationWriter extends FlatFileWriter {
	...
	public FeatureLocationWriter(Entry entry, Feature feature, WrapType wrapType,
								 String featureHeader, String qualifierHeader) {
	...
	public boolean write(Writer writer) throws IOException {
		...
		writeBlock(writer,
				featureHeader +
						feature.getName() +	getFeaturePadding(feature.getName()),
				qualifierHeader,
				block);
		return true;
	}
	...
	public static void renderLocation(StringBuilder block,
									  Location location, boolean leftPartial,
									  boolean rightPartial) {
		renderLocation(block, location, leftPartial, rightPartial, true);
	}

	public static void renderLocation(StringBuilder block,
									  Location location, boolean leftPartial,
									  boolean rightPartial,
									  boolean renderSingleBaseRangeAsSingleBase) {
		...
		if (location instanceof RemoteLocation &&
				((RemoteLocation)location).getAccession() != null) {
			// Remote location.			
		...
		if (location instanceof Base &&
			...
			renderBase(block, location.getBeginPosition(),
					isComplement, leftPartial, rightPartial);
			...
			renderBase(block, location.getBeginPosition(),
				...
				location.getBeginPosition() != null	&&
				location.getEndPosition() != null &&
				location.getBeginPosition().equals(location.getEndPosition()) && renderSingleBaseRangeAsSingleBase) {
			renderBase(block, location.getBeginPosition(),
					isComplement, leftPartial, rightPartial);
		} else if (location instanceof Range &&
				location.getBeginPosition() != null	&&
				location.getEndPosition() != null) {
			renderRange(block, location.getBeginPosition(), location.getEndPosition(),
					isComplement, leftPartial, rightPartial);
		} else if (location instanceof Between) {
			renderBetween(block, location.getBeginPosition(), location.getEndPosition(),
					isComplement, leftPartial, rightPartial);
	...
	private static void renderBase(StringBuilder block,
								   Long position,
	...
	private static void renderRange(StringBuilder block,
									Long beginPosition, Long endPosition,
									boolean isComplement,
									boolean leftPartial, boolean rightPartial) {
		renderRangeOrBetween(block, beginPosition, endPosition, isComplement,
				leftPartial, rightPartial, "..");
	}

	private static void renderBetween(StringBuilder block,
									  Long beginPosition, Long endPosition,
									  boolean isComplement,
									  boolean leftPartial, boolean rightPartial) {
		renderRangeOrBetween(block, beginPosition, endPosition, isComplement,
	...
	private static void renderGap(StringBuilder block,
								  Gap gap) {
	...
	private static void renderRangeOrBetween(StringBuilder block,
											 Long beginPosition, Long endPosition,
											 boolean isComplement,
											 boolean leftPartial, boolean rightPartial, String separator) {
		// Partiality.
		if (isComplement) {
			if (rightPartial) {
				block.append("<");
