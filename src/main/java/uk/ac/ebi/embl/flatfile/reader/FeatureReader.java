...
public class FeatureReader extends FlatFileLineReader {
  ...
  protected void readLines() throws IOException {
    ...
    if ((Feature.SOURCE_FEATURE_NAME.equals(feature.getName()) && skipSource)
        || (!Feature.SOURCE_FEATURE_NAME.equals(feature.getName())
            && lineReader.getReaderOptions() != null
            && lineReader.getReaderOptions().isParseSourceOnly())) {
      while (true) {
        lineReader.readLine();
        ...
        if (isReducedFlatfile) {
          ...
          if (!(feature instanceof SourceFeature)) {
            ...
            if (!FlatFileUtils.isBlankString(value)
              ...
              error(
                  "FT.7", qualifier.getName(), value, oldValue); // Inconsistent feature qualifier.
            } else {
  ...
  private CompoundLocation<Location> readLocation(String locationString) throws IOException {
    ...
    FeatureLocationParser locationParser = new FeatureLocationParser(this, lineReader.getReaderOptions().isIgnoreParserErrors());
  ...
  private Qualifier readQualifier() throws IOException {
    ...
    boolean noAddSpace =
        qualifierString.length() > 0 && qualifierString.charAt(qualifierString.length() - 1) == '-';
    boolean addSpace =
