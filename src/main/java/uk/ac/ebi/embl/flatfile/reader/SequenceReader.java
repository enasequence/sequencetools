...
public class SequenceReader extends FlatFileLineReader {
    ...
    readLines() throws IOException {
        ...
        final Consumer<byte[]> compactor = Entry.PROTEIN.equals(molType) ? this::compactProteinSequence : this::compactDNASequence;
        ...
        while (true) {
            ...
            if (sequence.remaining() < line.length()) {
                if (sequence.capacity() >= MAX_SEQUENCE_LENGTH) {
                    throw new IOException("Maximum supported sequence size " + MAX_SEQUENCE_LENGTH + " exceeded");
                }

                byte[] biggerArray = Arrays.copyOf(sequence.array(),
                        Math.min(MAX_SEQUENCE_LENGTH, Math.max((int) (sequence.capacity() * 1.7), sequence.capacity() + line.length())));
        ...
        entry.getSequence().setSequence(ByteBuffer.wrap(Arrays.copyOf(sequence.array(), sequence.position())));
