...
public class BookWriter extends FlatFileWriter {
	...
	public BookWriter(Entry entry, Book book, WrapType wrapType,
			String header) {
	...
	public boolean write(Writer writer) throws IOException {
		...
		writeBlock |=
			new BookEditorWriter(entry, book, wrapType,
					firstLineHeader, header).write(writer);
		writeBlock |=
			new BookChapterWriter(entry, book, wrapType,
					header, header).write(writer);
		writeBlock |=
			new BookPublisherWriter(entry, book, wrapType,
					header, header).write(writer);
		return writeBlock;
	}
}
