...
public class EntryContigsCheck extends EntryValidationCheck
{
	...
	private final static String CONTIG_EXISTS_MESSAGE_ID = "ContigEntryCheck-1";
	private final static String CONTIG_LOCATION_MESSAGE_ID = "ContigEntryCheck-2";
	private final static String CONTIG_LOCATION_MESSAGE_ID_1 = "ContigEntryCheck-3";
	private final static String GAP_CONTIG_MESSAGE_ID = "ContigEntryCheck-4";
	...
	public ValidationResult check(Entry entry) throws ValidationEngineException
	{
		...
		if(entry.getSequence().getContigs().get(0) instanceof Gap||entry.getSequence().getContigs().get(entry.getSequence().getContigs().size()-1) instanceof Gap)
		{
			...
			{
				if(assemblyGapFeature.getLocations().getMinPosition()==1||assemblyGapFeature.getLocations().getMaxPosition()==entry.getSequence().getLength())
				{
					if("paired-ends".equals(SequenceEntryUtils.getQualifierValue(Qualifier.LINKAGE_EVIDENCE_QUALIFIER_NAME, assemblyGapFeature)))
					{
