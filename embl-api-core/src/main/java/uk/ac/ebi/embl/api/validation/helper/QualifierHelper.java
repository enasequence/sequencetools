...
public class QualifierHelper {
  ...
  public static Optional<ValidationMessage<Origin>> checkNoValue(Qualifier qualifier, boolean noValue, String featureName) {
      ...
      if (!noValue && (value == null || value.equals(""))) {
          message =  Optional.of(EntryValidations.createMessage(qualifier.getOrigin(), Severity.ERROR, NO_VALUE_ID, qualifier.getName(), featureName));
      } else if (noValue && value != null && !value.isEmpty()) {
          message =  Optional.of(EntryValidations.createMessage(qualifier.getOrigin(), Severity.ERROR, NO_VALUE_ID_2, qualifier.getName(), featureName));
      }
      return message;
  }
  ...
  public static ValidationResult checkRegEx(Qualifier qualifier, QualifierInfo qualifierInfo) {
      ...
      if(null != qualifier.getValue() && null != qualifierInfo.getRegex()) {
        ...
        if (!matcher.matches()) {
            if (!isAlternateRegexMatch(qualifier)) {
                ValidationMessage<Origin> message = EntryValidations.createMessage(qualifier.getOrigin(), Severity.ERROR, REGEX_FAIL_ID, qualifier.getName(), qualifier.getValue(), qualifierInfo.getRegex());
                if (qualifierInfo.getCuratorComments() != null) {
                    message.setCuratorMessage(qualifierInfo.getCuratorComments());
                }
                result.append(message);
            }
        } else if (!qualifierInfo.getRegexGroupInfos().isEmpty()) {//there are specific group requirements
            for (QualifierHelper.RegexGroupInfo groupInfo : qualifierInfo.getRegexGroupInfos()) {
                ...
                if ((group == null && !groupInfo.canBeNonMatch()) || (group != null && !valuesContains(validValues, group, groupInfo.isCaseInseneitive()))) {
                    if (!"inference".equals(qualifier.getName()) && !qualifier.getValue().equalsIgnoreCase("non-experimental evidence, no additional details recorded")) {
                        ValidationMessage<Origin> message = EntryValidations.createMessage(qualifier.getOrigin(), Severity.ERROR, REGEX_GROUP_FAIL_ID, qualifier.getName(), group);
    ...
    public static List<RegexGroupInfo> setNullGroupTolerance(QualifierInfo qualifierInfo) {
        ...
        return  qualifierInfo.getRegexGroupInfos().stream().map(rgInfo -> { if(rgInfo.getGroupNumber()==3) { rgInfo.setNonMatch(true); } return rgInfo;}).collect(Collectors.toList());
            ...
            for (DataRow dataRow : dataRows) {
                ...
                if (!qualifierName.equals("EC_number")) { //EMD-2496
                    ...
                    if(null == regexpRows) {
                        ...
                        for (DataRow regexpRow : regexpRows) {//look at all the qualifier values associatesd with a regexp group
