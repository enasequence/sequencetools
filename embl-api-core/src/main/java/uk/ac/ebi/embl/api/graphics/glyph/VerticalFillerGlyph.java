...
public class VerticalFillerGlyph extends SimpleGlyph {
    public VerticalFillerGlyph(Canvas canvas) {
    	super(canvas);
    }

    public VerticalFillerGlyph(Canvas canvas, int height) {
    	super(canvas);
    	this.height = height;
    }
    
    private int height = VerticalSpacerGlyph.DEFAULT_HEIGHT;
    ...
    protected void drawSimpleGlyph(Graphics2D g) {
        ...
        rectGlyph.setWidth(getCanvas().getMainPanelWidth());
        rectGlyph.setHeight(getHeight());    	
        rectGlyph.drawGlyph(g);
    }
