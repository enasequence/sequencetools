...
{
	private final static Pattern CITATION_PATTERN = Pattern.compile("[^\\d]*(\\d+)[^\\d]*");
	private final static int CITATION_GROUP = 1;	
	public final static String CITATION_QUALIFIER_MESSAGE_ID = "CitationQualifierCheck_1";
	...
	public ValidationResult check(Entry entry)
	{
		...
		List<Qualifier> citationQualifiers=SequenceEntryUtils.getQualifiers(Qualifier.CITATION_QUALIFIER_NAME,entry);
		
		if(citationQualifiers.size()==0)
		  return result;
		...
		for(Qualifier citationQualifier: citationQualifiers)
		{
			...
			{	
			  ...
			  if(!referenceNumbers.contains(new Integer(qualifierReferenceNumber)))
			  reportError(citationQualifier.getOrigin(), CITATION_QUALIFIER_MESSAGE_ID,citationQualifier.getValue());
		    }
		}
