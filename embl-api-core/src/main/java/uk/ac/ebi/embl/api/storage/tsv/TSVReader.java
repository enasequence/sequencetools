...
/**
 * A file with rows with tab (\t) separated values. Comment lines starting with
 * a hash (#) and empty lines will be ignored.
 */
public class TSVReader {
	...
	public DataSet readDataSetAsStream(String fileName) throws IOException {
		...
		if (stream == null) {
			throw new IOException("Failed to load stream for resource: "
					+ fileName);
		}
	...
	public DataSet readDataSetAsFile(String fileName) throws IOException {
		...
		if (!file.exists()) {
			throw new IOException("Failed to load file for resource: "
					+ fileName);
		}
