...
//    private boolean partial = false;
    ...
    private Map<Integer, TranslationException> translationExceptionMap =
        new HashMap<Integer, TranslationException>();
    ...
    /** If true then a degenerate start codon is translated to M using 
    ...
    /** If true then 3' partiality is removed when a stop codon is found at
      * the 3' end. */
    ...
    /** If true then a partial codon is removed after a stop codon at the 3'
      * end.*/
    ...
    private boolean applyTranslationException(Codon codon) {
        TranslationException translationException = 
            translationExceptionMap.get(codon.getPos());
    ...
    public ValidationResult configureFromFeature(CdsFeature feature,
                                                 TaxonHelper taxHelper,
                                                 Entry entry) throws ValidationException {
        ...
        if (sourceFeature != null) {
              Taxon taxon = null;
        		...
        		if (StringUtils.equals(organelle, "mitochondrion")
        			...
        			infoMessage=EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-12",organelle,translationTable);
        			...
        			infoMessage=EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-11",translationTable);
        		}
              }
				...
				if (featureTranslationTable != null)
				{
					...
					infoMessage=EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-13",translationTable);
				}
				else {
					String organelle = sourceFeature.getSingleQualifierValue("organelle");
					if (StringUtils.equals(organelle, "mitochondrion")) {
						translationTable = 2;
						infoMessage=EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-14",organelle,translationTable);
					}
					else if (StringUtils.equals(organelle,"mitochondrion:kinetoplast")) {
						translationTable = 4;
						infoMessage=EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-14",organelle,translationTable);
					} else if (StringUtils.contains(organelle, "plastid")) {
						translationTable = TranslationTable.PLASTID_TRANSLATION_TABLE;
						...
						infoMessage=EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-15",translationTable);
					}
        ...
        if(featureTranslationTable!=null&&!featureTranslationTable.equals(translationTable))
        {
        	validationResult.append(EntryValidations.createMessage(entry.getOrigin(), Severity.ERROR, "CDSTranslator-10",featureTranslationTable,translationTable));
			translationTable=featureTranslationTable;
        }
        ...
        if (translationTable == null)
		{
			if (feature != null)// CDSfeature
				validationResult.append(EntryValidations.createMessage(feature.getOrigin(), Severity.INFO, "CDSTranslator-8",TranslationTable.DEFAULT_TRANSLATION_TABLE));
			else
				// non-cds feature
				validationResult.append(EntryValidations.createMessage(entry.getOrigin(), Severity.INFO, "CDSTranslator-8"));
        ...
        /**
         * set the left and right partiality - if the compound location is global complement, and one end is partial
         * only, we need to reverse the left and right partiality, as the translator assumes sequence to be on the
         * primary strand. (if both ends are partial or both are unpartial, no need to reverse)
         */
       ...
       if (compoundLocation.isComplement() &&
        ...
        for (TranslExceptQualifier qualifier : translExceptQualifiers) {
            ...
            Integer relativeEndPos =
                    feature.getLocations().getRelativeIntPosition(endPosition);
			if (relativeBeginPos != null && relativeEndPos != null)//EMD-5594
			{
				if (beginPosition < endPosition && relativeBeginPos > relativeEndPos)
				{
            ...
            if (relativeBeginPos == null) {
                validationResult
                        .append(EntryValidations.createMessage(feature.getOrigin(), Severity.ERROR, "CDSTranslator-6",
                                beginPosition.toString()));
            } else if (relativeEndPos == null) {
                validationResult
                        .append(EntryValidations.createMessage(feature.getOrigin(), Severity.ERROR, "CDSTranslator-7",
                                endPosition.toString()));
            } else {
                addTranslationException(relativeBeginPos,
                        relativeEndPos,
        ...
        feature.getComplexQualifiers(Qualifier.CODON_QUALIFIER_NAME, codonQualifiers);
    ...
    private Integer getEntryTranslationTable(Integer featureTranslationTable,TaxonHelper taxHelper,Entry entry,ValidationResult validationResult) throws ValidationException
    {
       	       Integer translationTable =null;
    ...
    public ExtendedResult<TranslationResult> translate(byte[] sequence, Origin origin) {
    	...
    	ExtendedResult<TranslationResult> extendedResult = new ExtendedResult<TranslationResult>(translationResult);
    ...
    private void validateCodonStart(int bases)
    ...
    private void validateCodons(int bases, TranslationResult translationResult)
        ...
        if (bases < 3) {
            ...
            //non mod 3 peptide features other than Cds dont throw errors as there are more complex checks in
            // PeptideFeatureCheck
    ...
    private void validateStopCodonOnly(TranslationResult translationResult)
        ...
        if (translationResult.getCodons().size() == 1 &&
            translationResult.getTrailingBases().length() == 0 && leftPartial) {
                return; // no conceptual translation
        }
        else {
    ...
    private boolean validateStartCodon(TranslationResult translationResult)
        ...
        if (!leftPartial && !exception && !peptideFeature) {
            if (!translationResult.getCodons().get(0).getAminoAcid().equals('M') &&
                !leftPartial ) {
                if (nonTranslating) {
                    return false; // no conceptual translation
                }
                    ...
                    if (fixMissingStartCodon) {
                    	...
                    	// The protein translation does not start with a methionine.
	...
	public boolean equalsTranslation(String expectedTranslation, String conceptualTranslation) {
	    ...
	    for ( int i = 0; i < conceptualTranslation.length(); i++ ) {
	        if (expectedTranslation.charAt(i) != conceptualTranslation.charAt(i) &&
	            expectedTranslation.charAt(i) != 'X') {
		        return false;
	        }
	    }
	    // Ignore trailing X.
