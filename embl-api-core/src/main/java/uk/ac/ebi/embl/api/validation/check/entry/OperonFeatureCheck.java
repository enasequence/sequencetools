...
public class OperonFeatureCheck extends EntryValidationCheck
{
	...
	protected final static String SINGLE_OPERON_MESSAGE = "OperonFeatureCheck_1";
	...
	public ValidationResult check(Entry entry)
	{
		...
		List<Qualifier> operonQualifiers = SequenceEntryUtils.getQualifiers(Qualifier.OPERON_QUALIFIER_NAME, entry);

		if (operonQualifiers != null && operonQualifiers.size() == 0)
		{
			return result;
		}
		List<Feature> operonFeatures = SequenceEntryUtils.getFeatures(Feature.OPERON_FEATURE_NAME, entry);
		...
		Collection<Feature> featuresWithOperon = SequenceEntryUtils.getFeaturesContainingQualifier(Qualifier.OPERON_QUALIFIER_NAME, entry);
		...
		if (featuresWithOperon != null)
		{
			...
			{
				reportError(entry.getOrigin(), SINGLE_OPERON_MESSAGE, ((Feature)featuresWithOperon.toArray()[0]).getName(),((Feature)featuresWithOperon.toArray()[0]).getSingleQualifierValue(Qualifier.OPERON_QUALIFIER_NAME));
			} else
			{
				...
				for(Feature feature1 :featuresWithOperon)
				{
					...
					int operonCount=0;
					if(operonCountMap.containsKey(operonValue))
						continue;
					for(Feature feature2:featuresWithOperon)
					{
						if(operonValue!=null&&operonValue.equals(feature2.getSingleQualifierValue(Qualifier.OPERON_QUALIFIER_NAME)))
						{
							operonCount++;
						}
					}
			 ...
			 {
				 reportError(entry.getOrigin(), MULTIPLE_OPERON_MESSAGE,operonCountMap.get(operonValue),operonValue);
			 }
