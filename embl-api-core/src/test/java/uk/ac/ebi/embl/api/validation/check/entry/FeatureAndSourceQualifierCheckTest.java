...
public class FeatureAndSourceQualifierCheckTest {
	...
	public void testCheck_MultipleSource() {
		source.setSingleQualifier("map");
		Feature source2 = featureFactory
				.createFeature(Feature.SOURCE_FEATURE_NAME);
	...
	public void testCheck_Message() {
		...
		ValidationResult result = check.check(entry);
		Collection<ValidationMessage<Origin>> messages = result.getMessages("FeatureAndSourceQualifierCheck-1", Severity.ERROR);
		assertEquals("Source qualifier map is required when feature STS exists.",messages.iterator().next().getMessage());
	}	
