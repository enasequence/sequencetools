...
public class GaptoAssemblyGapFeatureFixTest {
	...
	public void testCheck_withNoAssemblyGapFeatures() {
        ...
        assertEquals(1, SequenceEntryUtils.getFeatures(Feature.ASSEMBLY_GAP_FEATURE_NAME, entry).size());
		...
		Collection<ValidationMessage<Origin>> messages = validationResult
				.getMessages("GaptoAssemblyGapFeatureFix_1", Severity.FIX);
	...
	public void testCheck_withNoAssemblyScope() throws SQLException {
		...
		ValidationResult validationResult = check.check(entry);
		assertTrue(validationResult.isValid());
        assertEquals(0, SequenceEntryUtils.getFeatures(Feature.ASSEMBLY_GAP_FEATURE_NAME, entry).size());
	...
	public void testCheck_withAssemblyGapandGapFeatures() {
		...
		assertEquals(1,messages.size());
		assertEquals("GAP feature replaced with ASSEMBLY_GAP feature as entry has both gap and assembly_gap features. gap and assembly_gap features are mutually exclusive.",
