...
public class GeneSynonymFixTest {
    ...
    /**
     * a cds feature has its gene_synonyms set as the master synonyms, which are added to a feature with the
     * same gene qualifier
     */
    @Test
    public void testFix1() {
        ...
        ValidationResult result = check.check(entry);
        Collection<ValidationMessage<Origin>> messages = result.getMessages("GeneSynonymFix", Severity.FIX);
         ...
         */
        assertTrue(fixMessage.equals(messages.iterator().next().getMessage()) ||
                fixMessage.equals(messages.iterator().next().getMessage()));
    }

    /**
     * a cds feature has its gene_synonyms set as the master synonyms, which are added to a feature with the
     * same gene qualifier. An additional qualifier is removed from the non-master feature
     */
    @Test
    public void testFix2() {
        ...
        ValidationResult result = check.check(entry);
        Collection<ValidationMessage<Origin>> messages = result.getMessages("GeneSynonymFix2", Severity.FIX);
    ...
    /**
     * a cds feature has its gene_synonyms set as the master synonyms, which are added to a feature with the
     * same gene qualifier. An additional qualifier which also has synonyms is not treated as master due to not
     * being CDS
     */
    @Test
    ...
    /**
     * no CDS feature, so the first feature with synonyms is used as master, these synonyms are added to a feature with the
     * same gene qualifier.
     */
    @Test
    ...
    /**
     * a cds feature has its gene_synonyms set as the master synonyms, which are added to a feature with the
     * same *locus_tag* qualifier - treated same as gene. An additional qualifier which also has synonyms is not treated as master due to not
     * being CDS
     */
    @Test
    public void testFix5() {
        ...
        /**
         * this one has a locus tag, rather than a gene, but has the same value as gene so should be recognized
         */
        Feature feature3 = featureFactory.createFeature("feature3");
    ...
    /**
     * Two CDS features with same gene qualifier have different gene_synonyms - this means we cant decide which list to use
     * so will not attempt any fixes
     */
    @Test
